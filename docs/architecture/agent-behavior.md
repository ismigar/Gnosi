...
Additional validation on 2026-09-25 used the configured OpenRouter model (meta-llama/llama-4-maverick) with synthetic sources in an isolated Vault. Custom skill replacement, structured output, trace capture, JSON tool transport and complete reading with original quotations passed. The frozen backend passed process-specific readiness and multi-Vault discovery/read/switch checks. The macOS application was packaged and its code signature verified. Browser checks exercised all five behavior views in Catalan, Spanish, English and French with light and dark themes (40 view checks). Local installation provenance is recorded in the application’s reconciled-build.json manifest.

...
Les schémas de génogrammes chargent
`backend/domains/genograms/option_labels.json` au démarrage. Le plan explicite
de ressources inclut ce JSON examiné à côté de son module. Un test avec le plan
déplacé crée les schémas de personnes et de relations dans les quatre langues
sans accès au code source original ; l’exécutable empaqueté réel doit également
réussir le test de démarrage sur chaque plateforme.
...
## Correction de l’installation après 3.0.0

Electron empaqueté choisit un port local libre. Python accepte `BACKEND_PORT`
avec une `GNOSI_DESKTOP_INSTANCE` valide, écoute sur `127.0.0.1` et désactive le
rechargement. Les services natifs conservent leurs ports. Le test de démarrage
fournit une configuration contradictoire pour vérifier que le port du parent
prévaut et exige toujours l’identité du processus enfant. Un second lancement
recrée la fenêtre principale lorsqu’elle a été fermée.

Les tracés du favicon, indépendants des polices, génèrent les icônes dans
`desktop/assets`. Le DMG utilise un fond uni et une fenêtre de 540 × 320. Le
disque du bureau disparaît après éjection ; ce n’est pas un raccourci. La
signature locale gratuite reste utilisée et ne vaut pas notarisation Apple.
...
Un choix natif réussi enregistre aussi un `selectionId` opaque. Le preload isolé
l’applique avant les requêtes de l’interface : il efface uniquement l’ID, le slug,
le nom, le catalogue et le cookie de l’ancienne bibliothèque active. Les autres
préférences et cookies sont conservés. Le marqueur est appliqué une seule fois
par profil ; les changements normaux de bibliothèque survivent aux nouvelles
fenêtres et aux redémarrages. Les anciens fichiers sans marqueur restent valides.
Annuler la récupération conserve le choix précédent. Choisir un autre dossier
ne réécrit pas les identités enregistrées. Répéter l’acceptation du DMG pour le
...
La constante du backend `GNOSI_VERSION` indique également `3.1.0` ; le test du contrat de l’API de santé la compare aux quatre manifestes de version.
...
La version candidate `3.1.0` regroupe l’assistant principal commun, la sélection de modèles selon la tâche, la mémoire à portée définie, l’apprentissage réutilisable et la lecture contextuelle des sources. Les données existantes sont préservées par les migrations révisées. Tous les manifestes et `GNOSI_VERSION` concordent ; le contrat OpenAPI régénéré ne change que sa version par rapport au code préparé. Les deux fichiers de dépendances verrouillées restent inchangés. Le catalogue reste non publié jusqu’à ce que les installateurs issus du même code réussissent les tests d’installation initiale, de persistance et de mise à niveau sur les quatre cibles.

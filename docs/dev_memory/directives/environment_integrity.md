...
7. **Decommission** (només si QA passa): reescriure `com.gnosi.boot` per arrencar serveis natius (LaunchAgent uvicorn + vite); **eliminar `com.gnosi.docker-watchdog` i `com.gnosi.onedrive-warmup`** (ja no calen); `docker compose down` (SENSE `-v`); recuperar 16,9 GB d'imatges + Docker.raw; actualitzar CLAUDE.md (fi de la doctrina "Docker first"). **[Executat 2026-06-17 amb dues desviacions — vegeu «Estat post-migració» a sota.]**

### Estat post-migració + regles dual-mode (actualitzat 2026-07-15)
- **Fet:** Docker Desktop DESINSTAL·LAT d'aquest Mac (imatges + Docker.raw recuperats, ~19 GB); `com.gnosi.boot` i `com.gnosi.docker-watchdog` esborrats del tot (NO queden `.plist.disabled`). LaunchAgents vius: `com.gnosi.backend`, `com.gnosi.frontend`, `com.gnosi.onedrive-warmup`, `com.gnosi.host-open-helper`.
- **Desviació 1 (deliberada): `com.gnosi.onedrive-warmup` ES CONSERVA.** El pla deia eliminar-lo, però el daemon :5009 continua sent l'eina de materialització de fitxers `dataless` dels runbooks de recuperació (incidents de juliol: `BD/` i `page_meta` online-only). El backend natiu ja no en depèn per al warmup automàtic (mode `open` autodetectat des del PR #838).
- **Desviació 2: `com.gnosi.host-open-helper` també es conserva** — dona Spotlight (`/search`), obrir amb l'app per defecte (`/open`) i Paperera (`/trash`) al backend en tots dos modes.
- **Docker = mode de desplegament suportat (self-host), NO fallback local.** La recepta (`docker-compose.yml`, `Dockerfile.*`) la vigila el workflow `.github/workflows/docker-build.yml`: build de les dues imatges + `docker compose config` + smoke test `/api/health` amb `GNOSI_FILES_PROVIDER=local` (per PR que toqui la recepta + execució setmanal).
- **Prereqs de HOST per màquina (natiu):** `pandoc` no viatja amb git — cal `brew install pandoc` a CADA Mac (aquest Mac: instal·lat 2026-07-15; la nota del 06-07 es va fer des de l'altre Mac). `pdflatex` (MacTeX) segueix pendent/opcional per a l'export PDF.

#### Restriccions/Edge Cases (dual-mode Docker+natiu)
- **No deixis un default només-Docker** (`host.docker.internal`, rutes de contenidor `/vault` o `/app/data`) escrit en dur al codi → en natiu no resol i la funcionalitat degrada EN SILENCI (warmup: `ConnectError` a cada materialització i EDEADLK persistent — PR #838; helper :5099: la cerca Spotlight queia a `os.walk` i la Paperera retornava 502 — corregit al mateix PR que aquesta secció) → **usa autodetecció amb `_is_docker()`** (`backend/config/env_config.py`): `default_host_helper_url()` per al helper i `_default_warmup_mode()`/`_default_warmup_url()` a `files_provider/onedrive.py`. Les env vars (`GNOSI_HOST_*_HELPER_URL`, `ONEDRIVE_WARMUP_*`) sempre queden com a override.
- **No afegeixis dependències de sistema al backend sense fer-les dual-mode** → la imatge Docker les porta (`Dockerfile.backend`: pandoc, ghostscript) però el HOST natiu no → documenta-les com a prereq del runbook I resol-les tolerant PATH mínims (patró `_pandoc_bin()`: env override → `which` → rutes Homebrew).
- **No deixis que un walk recursiu del vault (`os.scandir`/`rglob`) propagui `OSError` EDEADLK/EAGAIN** → sota launchd el File Provider NO hidrata on-access i UN subdirectori encallat (p. ex. `BD/Cervell` online-only) tombava TOT l'endpoint (GET `/api/graph` → 500, errors `[1f98a0d9]`/`[eab0704c]` 2026-07-16) → patró **"salta i registra" per directori** (mateix que sidecars dataless 9ddd99ede i indexadors de warmup 8b06db765): captura amb les constants `errno.EDEADLK`/`errno.EAGAIN` (els valors 11/35 estan INTERCANVIATS entre macOS i Linux — mai literals), continua el walk, marca el resultat `partial: true` i **NO el cachegis com a bo** (un graf quasi buit quedaria clavat fins al següent TTL). En mode natiu, demana la hidratació del directori amb `open -g -j <dir>` (Finder, background) amb throttle — vegeu `get_markdown_files_efficient` + `_request_dir_warmup` a `graph_service.py` i el test `test_graph_wedged_dirs.py`.

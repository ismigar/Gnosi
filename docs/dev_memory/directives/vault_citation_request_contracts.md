...
- Missing fields and explicit null retain the same truthiness-based defaults as
  2.x.
- A readonly frontend identifier array must be copied at the JSON serialization
  boundary. Passing it directly to the mutable array generated by OpenAPI fails
  strict TypeScript; spreading it preserves omission, order and values.

...
- **Clic = selecciona** la cel·la; **clic sobre la cel·la ja activa o doble-clic = edita el títol inline** (mateix model que la resta de cel·les). **Obrir la fitxa NO es fa des del títol**: només amb els botons de l'esquerra (icona obrir) o `Alt+O`. La fila (`<tr>`) manté el seu `onDoubleClick=onNoteSelect`, però el `<td>` del títol fa `stopPropagation`, així que el doble-clic sobre el títol edita i no obre. (Decisió **2026-06-03**, a petició de l'usuari, que **substitueix** la de 2026-05-25 «clic=selecciona / doble-clic=obre»: en treure el doble-clic-obre del títol desapareix el xoc que abans impedia aplicar-hi «clic sobre activa = edita». Implementat amb `openTitleEditor()`, paral·lel a `openEditor()` de les metadades.)

...
| --- | --- | --- | --- |
| 26/02 | `${workspaceFolder}/monorepo/apps/gnosi/.venv/bin/python` could not be resolved | Variable resolution failure in a specific IDE state | Use VS Code's `${userHome}` variable for an absolute portable path; `~` is not expanded in settings: `${userHome}/Projectes/monorepo/apps/gnosi/.venv/bin/python` |
| 28/03 | `ERROR MCP: drupal-proxy: error: No such file or directory` | Incorrect path in `mcp_config.json` | Updated `mcp_config.json` to use `${userHome}` rather than unexpanded `~`: `${userHome}/Projectes/monorepo/apps/gnosi/mcp-servers/drupal-proxy` |

...
## Objective
Develop a unified interface to manage social media accounts (Hootsuite style) within the Gnosi ecosystem.
...
1.  **Frontend:** Use existing `monorepo/apps/gnosi/frontend`. (React + Vite).
2.  **Backend:** Use existing `monorepo/apps/gnosi/backend` (FastAPI).

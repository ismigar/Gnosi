...
## 1. Objectiu
Proveir una eina nativa al Gnosi de lectura lliure de distraccions, orientada a la recepció de fonts externes (Feeds RSS, Newsletters) i transformació a àudio periòdic, fomentant la reducció de fatiga visual per la lectura llarga.

## 2. Components

### Base de Dades Local (SQLite)
Tot i que el Gnosi històricament utilitzava JSONs, `reader.db` introdueix SQLAlchemy com a prova de base de dades SQL local. Les entitats principals són:

...
## Description
The Gnosi backend contains an internal background task scheduler managed by `SchedulerManager` (located in `backend/scheduler/manager.py`). It currently orchestrates Reader flows (`fetch_feeds`, `fetch_newsletters`, `generate_podcast`) and operational flows (`sync_directives`, `backup_tools`, `cleanup_rejected`, `update_analytics`).
...
- **Config Migration on New Tasks**: If `scheduler_config.json` already exists from older versions, missing tasks must be auto-added from `AVAILABLE_TASKS` during load. Note: Do not overwrite existing task settings when adding missing entries, because this resets user toggles/intervals; instead, merge only absent tasks.

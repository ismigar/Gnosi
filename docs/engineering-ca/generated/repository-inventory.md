<!-- Generated by pipeline/skills/technical_documentation/scripts/generate.py. Do not edit manually. -->

# Inventari del repositori

High-level snapshot of the authoritative application tree. Generated and vendored dependencies are excluded where the catalog can identify them reliably.

Regenera amb `python pipeline/skills/technical_documentation/scripts/generate.py`.

## Recomptes principals

| Àrea | Recompte |
| --- | ---: |
| Backend Python files | 1285 |
| Backend Python test files | 486 |
| Frontend JS/TS source files | 1862 |
| Frontend unit test files | 519 |
| Registered FastAPI routers | 37 |
| Runtime skill contracts | 16 |
| Development-memory directives | 20 |

## Àrees pròpies de l’aplicació

| Àrea | Fitxers | Abast funcional |
| --- | ---: | --- |
| `backend/` | 1303 | FastAPI, services, models, agents, scheduling, and storage adapters |
| `frontend/src/` | 1941 | React application, UI behavior, state, and browser integrations |
| `pipeline/` | 102 | Reusable application skills and deterministic processing tools |
| `desktop/` | 99 | Desktop lifecycle, backend packaging, IPC, and updates |
| `extensions/` | 49 | Office, browser, plugin, marketplace, and external-system adapters |
| `tests/e2e/` | 42 | Host-level Playwright acceptance tests |
| `scripts/` | 34 | Native, self-host, release, and maintenance scripts |

## Límit d’exclusió

Catalogs exclude or avoid interpreting `.venv`, `node_modules`, `dist`, cache directories, Playwright reports, `local_data`, local SQLite files, and `frontend/vendor`. Vendored reader code is documented only at its integration boundary.

<!-- Generated by pipeline/skills/technical_documentation/scripts/generate.py. Do not edit manually. -->

# Repository inventory

High-level snapshot of the authoritative application tree. Generated and vendored dependencies are excluded where the catalog can identify them reliably.

Regenerate with `python pipeline/skills/technical_documentation/scripts/generate.py`.

## Key counts

| Surface | Count |
| --- | ---: |
| Backend Python files | 395 |
| Backend Python test files | 167 |
| Frontend JS/TS source files | 307 |
| Frontend unit test files | 45 |
| Registered FastAPI routers | 33 |
| Runtime skill contracts | 23 |
| Development-memory directives | 165 |

## Owned application surfaces

| Surface | Files | Purpose boundary |
| --- | ---: | --- |
| `backend/` | 405 | FastAPI, services, models, agents, scheduling, and storage adapters |
| `frontend/src/` | 327 | React application, UI behavior, state, and browser integrations |
| `pipeline/` | 99 | Reusable application skills and deterministic processing tools |
| `electron/` | 18 | Desktop lifecycle, backend packaging, IPC, and updates |
| `integrations/` | 12 | Office and external-system adapters |
| `web-clipper/` | 11 | Browser extension capture surface |
| `e2e/` | 26 | Host-level Playwright acceptance tests |
| `sh/` | 22 | Native and self-host operational scripts |

## Exclusion boundary

Catalogs exclude or avoid interpreting `.venv`, `node_modules`, `dist`, cache directories, Playwright reports, `local_data`, local SQLite files, and `frontend/vendor`. Vendored reader code is documented only at its integration boundary.

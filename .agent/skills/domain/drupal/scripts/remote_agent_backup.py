...
class DrupalRemoteAgent:
    ...
    def run_command(self, cmd, timeout=30):
        """Ejecuta un comando en el servidor remoto, gestionando suweb."""
        ...
        try:
            ...
            # Añadimos 'ismigar@' o similar para detectar prompts custom
            ...
            if i == 0: # Pide password
                ...
                return False
            elif i >= 4: # Ya estamos dentro (prompt detectado)
                print("✅ Login SSH automático (key-based login?)")

            # Esperar prompt post-login explícitamente si enviamos password
            if i <= 1:
                idx = child.expect(['\$', '#', '>', '%', 'ismigar@', pexpect.TIMEOUT])
                if idx == 5:
                    print("⚠️ Timeout esperando prompt tras login. Buffer:")
                    print(child.before)
            
            # 2. Gestionar suweb
            if self.suweb_pass:
                print("🔑 AGENT: Ejecutando suweb...")
                ...
                # Esperamos explícitamente Password:, o si falla, un prompt de root/error
                # IMPORTANTE: No incluir '$' genérico aquí o matcheará el echo del comando
                j = child.expect(['assword:', 'Ontraseña:', 'ontrasenya:', 'su: Authentication failure', pexpect.TIMEOUT], timeout=10)
                
                if j < 3: # Pidió password
                     child.sendline(self.suweb_pass)
                     # Esperar prompt final de suweb (debería ser diferente o root)
                     # Aumentamos timeout por si tarda en loguear
                     child.expect(['\$', '#', '>', '%', 'root@'], timeout=10)
                elif j == 3:
                     print("❌ Error: Fallo de autenticación en suweb previo?")
                     return False
                else:
                     print("⚠️ Timeout esperando 'Password:'. Verificando si ya somos root...")
                     # Verificar si estamos en shell verificando el prompt
                     child.sendline('echo "CHECK_ROOT"')
                     child.expect('CHECK_ROOT')
                     child.expect(['\$', '#', '>', '%'])

            # 3. Navegar
            print(f"📂 AGENT: Navegando a {self.drupal_root}")
            ...
            print(f"❌ Exception: {e}")
            if 'child' in locals():
                print("Buffer final:", child.before)
            return False

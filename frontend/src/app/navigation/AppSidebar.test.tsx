...
it('suspends background navigation for modal layers and open Settings', async () => {
    ...
    const navigate = async () => { await act(async () => { window.dispatchEvent(new KeyboardEvent('keydown', {key: '2', ctrlKey: true, bubbles: true, cancelable: true})); await Promise.resolve(); }); };
    ...
    await act(async () => { window.dispatchEvent(new KeyboardEvent('keydown', {key: '1', ctrlKey: true, bubbles: true, cancelable: true})); await Promise.resolve(); });

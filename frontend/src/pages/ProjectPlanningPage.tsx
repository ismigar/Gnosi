...
function projectTableIdFromSettings(settings: unknown): string | undefined {
    if (typeof settings !== 'object' || settings === null || Array.isArray(settings)) {
        return undefined;
    }
    const tableId = (settings as Record<string, unknown>).project_table_id;
    return typeof tableId === 'string' && tableId.trim() ? tableId : undefined;
}
...
const PlanningTimeline = VaultTimeline as unknown as ComponentType<
    ...
    return (
        ...
        <div className="mx-auto w-full max-w-7xl flex-1 space-y-4 overflow-y-auto p-4 sm:p-6">
            ...
            <section className="grid gap-4 lg:grid-cols-2"><article className="gnosi-panel p-4"><h2 className="mb-3 text-lg font-medium">{t('planning_page.baselines', 'Baselines')}</h2><div className="flex flex-wrap gap-2"><input value={baselineName} onChange={(event) => {
                ...
                setWorklog({ ...worklog, task_id: event.target.value });
            }} aria-label={t('planning_page.worklog_task', 'Work log task')} className="min-h-10 rounded border bg-[var(--bg-primary)]"><option value="">{t('planning_page.task', 'Task')}</option>{tasks.map((task) => <option key={task.id} value={task.id}>{task.title}</option>)}</select><input type="date" value={worklog.date} onChange={(event) => {
                setWorklog({ ...worklog, date: event.target.value });
            }} aria-label={t('planning_page.worklog_date', 'Work log date')} className="min-h-10 rounded border bg-[var(--bg-primary)]" /><input type="number" step="0.25" value={worklog.hours} onChange={(event) => {
                setWorklog({ ...worklog, hours: event.target.value });
            }} aria-label={t('planning_page.worklog_hours', 'Work log hours')} className="min-h-10 rounded border bg-[var(--bg-primary)]" /></div><button onClick={() => void addWorklog()} className="gnosi-button gnosi-button--secondary mt-2">{t('planning_page.add_worklog', 'Add work log')}</button><p className="mt-2 text-sm text-[var(--text-tertiary)]">{worklogs.length} {t('planning_page.entries', 'entries')}</p></article></section>
            <section className="grid gap-4 lg:grid-cols-2"><article className="rounded border bg-[var(--bg-primary)] p-4"><h2 className="mb-3 text-lg font-medium">{t('planning_page.resource_heatmap', 'Resource load')}</h2><div className="grid grid-cols-7 gap-1">{(allocation?.buckets ?? []).map((bucket) => <div key={`${bucket.resource_id}-${bucket.date}`} title={`${bucket.resource_name}: ${String(bucket.assigned_hours)}/${String(bucket.capacity_hours)} h`} className={`rounded p-2 text-xs ${bucket.overallocated_hours ? 'bg-red-500 text-white' : 'bg-emerald-100 text-emerald-900'}`}>{bucket.resource_name}<br />{bucket.date}<br />{bucket.assigned_hours}h</div>)}</div></article><article className="rounded border bg-[var(--bg-primary)] p-4"><h2 className="mb-3 text-lg font-medium">{t('planning_page.leveling', 'Resource leveling')}</h2><button onClick={() => void createProposal()} className="rounded border px-3 py-1">{t('planning_page.generate_proposal', 'Generate proposal')}</button>{proposal && <div className="mt-3 text-sm"><p>{proposal.proposals.length} {t('planning_page.proposed_changes', 'proposed changes')}</p><button onClick={() => void applyProposal()} className="mt-2 rounded bg-[var(--accent-primary)] px-3 py-1 text-white">{t('planning_page.apply_proposal', 'Apply proposal')}</button></div>}<ul className="mt-3 text-sm">{(allocation?.assignment_summaries ?? []).map((assignment) => <li key={assignment.id}>{assignment.task_id}: {assignment.planned_work_hours}h · {assignment.estimated_cost}</li>)}</ul></article></section>
            {diagnostics.length > 0 && <section className="rounded border bg-[var(--bg-primary)] p-4"><h2 className="mb-2 text-lg font-medium">{t('planning_page.diagnostics', 'Diagnostics')}</h2><ul className="space-y-1 text-sm">{diagnostics.map((item, index) => <li key={`${item.code}-${String(index)}`}>{item.message}</li>)}</ul></section>}
        </div>
        </div>
    );
}

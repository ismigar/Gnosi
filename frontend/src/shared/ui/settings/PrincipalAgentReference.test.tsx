...
vi.mock('../../api/configuration', () => ({ fetchConfiguration: () => Promise.resolve({ ai: { active_agent_id: 'personal', agents: [{ id: 'personal', name: 'My Brain' }, { id: 'builtin.feeds-reader.default', managed_by: 'builtin:feeds-reader', name: 'Podcast profile' }] } }) }));
...
describe('principal reference', () => {
    it.each(['ca', 'es', 'en', 'fr'])('shows the plugin profile in %s without a duplicate model selector', async language => {
        ...
        expect(container.textContent).toContain('Podcast profile');

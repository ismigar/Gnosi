...
const scrollCases: ReadonlyArray<readonly [string, number, number]> = [
  ['ArrowDown', 100, 140],
  ['ArrowUp', 100, 60],
  ['PageDown', 100, 340],
  ['PageUp', 300, 60],
...
describe('adaptive hover preview layout', () => {
  ...
  it('detects vertical overflow without depending on width', () => {
    expect(
      isHoverPreviewScrollable({ scrollHeight: 401, clientHeight: 400, scrollTop: 0 }),
    ).toBe(true);
    expect(
      isHoverPreviewScrollable({ scrollHeight: 400, clientHeight: 400, scrollTop: 0 }),
    ).toBe(false);
    expect(isHoverPreviewScrollable(null)).toBe(false);
  });

  it.each(scrollCases)(
    'scrolls an overflowing preview with %s',
    (key, initial, expected) => {
      const element = {
        scrollTop: initial,
        scrollHeight: 1000,
        clientHeight: 300,

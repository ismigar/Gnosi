...
const EventForm = lazy(() => import('./calendar-sidebar-right/EventForm').then(module => ({ default: module.EventForm })));
const AvailabilityTool = lazy(() => import('./calendar-sidebar-right/AvailabilityTool').then(module => ({ default: module.AvailabilityTool })));
    ...
    if (eventPanel) {
        return (
            <div className="w-72 flex-shrink-0 bg-[var(--bg-secondary)] border-l border-[var(--border-primary)] flex flex-col h-full overflow-hidden">
                <Suspense fallback={<div className="p-5 space-y-3">
                    <p role="status">{t('common.loading')}</p>
                    <button type="button" onClick={onClosePanel}>{t('common.close', 'Close')}</button>
                </div>}>
                    <EventForm
        ...
        <div className="w-64 flex-shrink-0 bg-[var(--bg-secondary)] border-l border-[var(--border-primary)] flex flex-col h-full overflow-hidden text-sm text-[var(--text-secondary)]">
            ...
            <div className="flex-1 overflow-y-auto">
                {activeTab === 'shortcuts' ? (
                    ...
                    <Suspense fallback={<p className="p-5" role="status">{t('common.loading')}</p>}>

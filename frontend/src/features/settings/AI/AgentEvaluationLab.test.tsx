...
vi.mock('../../../shared/api/ai-activity', () => ({
    fetchEvaluationAgents: async () => [{ id: 'a', name: 'Agent', provider: 'test', model: 'model' }],
...
describe('evaluation authorization', () => {
    it('requires selecting an enabled agent and explicit authorization before any calls', async () => {
        ...
        await act(async () => { if (agent) { agent.value = 'a'; agent.dispatchEvent(new Event('change', { bubbles:true })); } });
        expect(button?.disabled).toBe(true);

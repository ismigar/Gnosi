...
export function AgentEvaluationLab({ onComplete }: { onComplete?: () => void }) {
    ...
    return <section className="ai-resource-card" style={{ margin: '16px 0', padding: '16px' }}>
        ...
        {open && <div className="ai-resource-editor">
            ...
            {kind === 'role' ? <label>{t('agent_team.roles')}<select className="gnosi-select" value={role} disabled={busy} onChange={e => { setRole(e.target.value as EvaluationRequest['role']); setAuthorized(false); }}>{roles.map(r => <option key={r} value={r}>{t(`model_comparison.profiles.${r}`)}</option>)}</select></label> : <>
            ...
            {reports.slice(0, 10).map(report => <article className="ai-resource-card" key={report.id}>
                ...
                {report.kind === 'strategies' && <div className="model-table-wrap"><table className="model-comparison-table"><thead><tr>{['strategy', 'passed', 'calls', 'director_calls', 'avoidable', 'cost'].map(k => <th key={k}>{t(`agent_team.lab_${k}`)}</th>)}</tr></thead><tbody>{strategies.map(strategy => {
                    const cases = report.cases.filter(c => c.strategy === strategy);
                    const known = cases.every(c => c.cost_usd != null);
                    return <tr key={strategy}><td>{t(`agent_team.lab_${strategy}`)}</td><td>{cases.filter(c => c.passed).length}/{cases.length}</td><td>{cases.reduce((n, c) => n + c.model_calls, 0)}</td><td>{cases.reduce((n, c) => n + (c.director_calls ?? 0), 0)}</td><td>{cases.reduce((n, c) => n + (c.avoidable_director_calls ?? 0), 0)}</td><td>{known ? `$${cases.reduce((n,c) => n + (c.cost_usd ?? 0),0).toFixed(6)}` : t('agent_team.unknown')}</td></tr>;
                })}</tbody></table></div>}
                <details><summary>{t('agent_team.evidence')}</summary><ul>{report.cases.map(c => <li key={`${c.strategy}:${c.id}`}>{c.strategy && `${t(`agent_team.lab_${c.strategy}`)} · `}{c.id}: {t(c.passed ? 'agent_team.lab_pass' : 'agent_team.lab_fail')} · {c.model_calls} {t('agent_team.lab_calls')} · {c.latency_ms} ms · {c.cost_usd == null ? t('agent_team.unknown') : `$${c.cost_usd.toFixed(6)}`}</li>)}</ul></details>
            </article>)}
        </div>}
    </section>;
}

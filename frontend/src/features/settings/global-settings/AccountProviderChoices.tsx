...
export function AccountProviderChoices({ context }: Props) {
  ...
  return ((() => {
    const provider = accountProviderForEmail(addAccountEmail) ?? (isManualGoogle ? 'google' : null);
    if (!isCompleteAccountEmail(addAccountEmail) || (!provider && !addAccountEmailBlurred)) return null;
    ...
    const iconBox = (r?: string): CSSProperties => ({ background: 'white', padding: '7px', display: 'flex', flexShrink: 0, alignItems: 'center', justifyContent: 'center', borderRadius: r || '10px' });

    const GoogleBtn = () => (
      <button type="button" onClick={() => { window.location.href = googleSignInPath(activeTab, addAccountEmail); }} style={btnStyle('#4285f4', '0 8px 16px rgba(66,133,244,0.25)')}>
        ...
        <span>{tn('accounts.continue_with', { provider: 'Google' })}</span>
      </button>
    );
    const MicrosoftBtn = () => (
    ...
    const ICloudBtn = () => (
      <button type="button" onClick={() => { fillImap({ host: 'imap.mail.me.com', port: '993', enc: 'ssl' }, { host: 'smtp.mail.me.com', port: '587', enc: 'starttls' }); }} style={btnStyle('#555', '0 8px 16px rgba(0,0,0,0.15)')}>
        ...
        <span>{tn('accounts.continue_with', { provider: 'iCloud' })}</span>
      </button>
    );
    const YahooBtn = () => (
      <button type="button" onClick={() => { fillImap({ host: 'imap.mail.yahoo.com', port: '993', enc: 'ssl' }, { host: 'smtp.mail.yahoo.com', port: '465', enc: 'ssl' }); }} style={btnStyle('#6001d2', '0 8px 16px rgba(96,1,210,0.2)')}>
        ...
        <span>{tn('accounts.continue_with', { provider: 'Yahoo' })}</span>
      </button>
    );
    const AolBtn = () => (
      <button type="button" onClick={() => { fillImap({ host: 'imap.aol.com', port: '993', enc: 'ssl' }, { host: 'smtp.aol.com', port: '465', enc: 'ssl' }); }} style={btnStyle('#ff0b00', '0 8px 16px rgba(255,11,0,0.2)')}>
        ...
        <span>{tn('accounts.continue_with', { provider: 'AOL' })}</span>
      </button>
    );

    // Domain clearly identified → single button
    if (provider === 'google') return <div className="animate-in" style={{ marginTop: '8px' }}><GoogleBtn /></div>;
    if (provider === 'microsoft') return <div className="animate-in" style={{ marginTop: '8px' }}><MicrosoftBtn /></div>;
    if (provider === 'icloud') return <div className="animate-in" style={{ marginTop: '8px' }}><ICloudBtn /></div>;
    if (provider === 'yahoo') return <div className="animate-in" style={{ marginTop: '8px' }}><YahooBtn /></div>;
    if (provider === 'aol') return <div className="animate-in" style={{ marginTop: '8px' }}><AolBtn /></div>;

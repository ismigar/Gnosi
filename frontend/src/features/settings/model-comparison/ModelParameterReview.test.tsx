...
describe('parameter source review', () => {
    it('requires evidence and reloads after official verification', async () => {
        ...
        await act(async () => { root.render(<ModelParameterReview modelId="m" modelName="Model" onUpdated={updated} />); });
        ...
        await act(async () => { if (select) { select.value = 'not_published'; select.dispatchEvent(new Event('change', { bubbles: true })); } });

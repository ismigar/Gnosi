...
describe('orderGroupKeys', () => {
  it('reverses catalog groups in descending order and keeps the empty group last', () => {
    expect(
      orderGroupKeys({ keys, direction: 'desc', emptyKey: EMPTY }),
    ).toEqual(['PER FER', 'EN PROGRÉS', 'FET', EMPTY]);
  });
  ...
  it('sorts group labels alphabetically in both directions', () => {
    ...
    const getLabel = (key: string): string | undefined => labels[key];
    ...
    ).toEqual(['FET', 'EN PROGRÉS', 'PER FER', EMPTY]);
    expect(
      orderGroupKeys({
        keys,
        mode: 'alpha',
  ...
  it('sorts by record count in both directions', () => {
    ...
    ).toEqual(['EN PROGRÉS', 'PER FER', 'FET', EMPTY]);
    expect(
      orderGroupKeys({
        keys,
        mode: 'count',
    ...
    ).toEqual(['FET', 'PER FER', 'EN PROGRÉS', EMPTY]);
  });
});

...
describe('viewConstants', () => {
  describe('isPageEmbedView', () => {
    ...
    it('returns false for global database views without "this" filter', () => {
      ...
      expect(
        isPageEmbedView({ id: 'v2', name: 'Avui', type: 'table' }),
      ).toBe(false);
    });
  });
      ...
      const tableViews = [
        {
          ...
          name: 'Project Tasks',
          is_main: true,
          filters: [{ field: 'Projecte', value: 'this' }],
        },
  ...
  describe('isViewHidden', () => {
    ...
    it('never hides the main view', () => {
      ...
      expect(
        isViewHidden({ ...tableViews[0], hidden: true }, tableViews),
      ).toBe(false);
    });

...
function translate(key: string, fallback: string): string {
    const value = i18n.t(key, fallback);
...
export function buildSlashCommandCatalog({
    ...
    const tableItems = allTables.map(table => ({
        ...
        description: translate('editor.slash_add_table_view', "Add a view of this table to the page"),
        aliases: ['vault', 'vista', 'view', table.name].filter(isString),
        group: translate('editor.slash_group_knowledge', "Knowledge"),
        onItemClick: () => onOpenPageView(table.id),
    }));

    // Generic entry with no pre-selection
    const vistaItem = [{
        title: translate('editor.slash_view_title', "View"),
        description: translate('editor.slash_view_desc', "Add a filtered table view to this page"),
...
export function buildTurnIntoCatalog({ editor }: TurnIntoCatalogOptions = {}) {
    ...
    const targets: readonly TurnIntoTarget[] = [
        { type: 'paragraph', iconKey: 'paragraph', title: translate('editor.block_type_paragraph', "Paragraph"), aliases: ['paragraf', 'paragraph', 'text', 'p'] },
        { type: 'heading', props: { level: 1 }, iconKey: 'heading1', title: translate('editor.block_type_heading1', "Heading 1"), aliases: ['encapcalament', 'h1', 'titol', 'heading'] },
        { type: 'heading', props: { level: 2 }, iconKey: 'heading2', title: translate('editor.block_type_heading2', "Heading 2"), aliases: ['encapcalament', 'h2', 'subtitol', 'heading'] },
        { type: 'heading', props: { level: 3 }, iconKey: 'heading3', title: translate('editor.block_type_heading3', "Heading 3"), aliases: ['encapcalament', 'h3', 'heading'] },
        { type: 'heading', props: { level: 1, isToggleable: true }, iconKey: 'toggleHeading1', title: translate('editor.block_type_toggle_heading1', "Toggle heading 1"), aliases: ['encapcalament', 'desplegable', 'plegable', 'toggle', 'h1', 'collapsable'] },
        { type: 'heading', props: { level: 2, isToggleable: true }, iconKey: 'toggleHeading2', title: translate('editor.block_type_toggle_heading2', "Toggle heading 2"), aliases: ['encapcalament', 'desplegable', 'plegable', 'toggle', 'h2', 'collapsable'] },
        { type: 'heading', props: { level: 3, isToggleable: true }, iconKey: 'toggleHeading3', title: translate('editor.block_type_toggle_heading3', "Toggle heading 3"), aliases: ['encapcalament', 'desplegable', 'plegable', 'toggle', 'h3', 'collapsable'] },
        { type: 'bulletListItem', iconKey: 'bullet', title: translate('editor.block_type_bullet_list', "Bulleted list"), aliases: ['llista', 'list', 'pics', 'bullet', 'ul'] },
        { type: 'numberedListItem', iconKey: 'numbered', title: translate('editor.block_type_numbered_list', "Numbered list"), aliases: ['llista', 'numerada', 'numbered', 'ol'] },
        { type: 'checkListItem', iconKey: 'check', title: translate('editor.block_type_check_list', "Checklist"), aliases: ['llista', 'verificacio', 'check', 'todo', 'tasca'] },
        { type: 'toggle', iconKey: 'toggle', title: translate('editor.block_type_toggle', "Toggle"), aliases: ['alternanca', 'toggle', 'plegable', 'desplegable'] },
        { type: 'quote', iconKey: 'quote', title: translate('editor.block_type_quote', "Quote"), aliases: ['cita', 'quote', 'citacio'] },
        { type: 'codeBlock', iconKey: 'code', title: translate('editor.block_type_code', "Code"), aliases: ['codi', 'code', 'monospace'] },
    ];
    ...
    return targets.map(target => ({
        ...
        group: translate('editor.turn_into_group', "Turn into"),
        subtext: translate('editor.turn_into_subtext', "Convert the current block"),
        onItemClick: () => {
            if (!editor) return;
            try {
...
export function buildColumnLayoutCatalog({ editor }: ColumnLayoutCatalogOptions = {}) {
    ...
    return layouts.map(layout => ({
        title: layout.title,
        subtext: translate('editor.column_layout_subtext', "Insert a column layout"),

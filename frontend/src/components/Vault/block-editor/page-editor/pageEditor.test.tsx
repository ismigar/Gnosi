...
vi.mock('../../../../hooks/use-api', () => ({ useApi: () => ({ role: fixture.role }) }));
vi.mock('../../../../plugins/usePlugins', () => ({ usePlugins: () => ({ isEnabled: () => false, getPluginSettings: () => fixture.planning }) }));
vi.mock('../../../../hooks/useTheme', () => ({ useTheme: () => ({ effectiveTheme: 'light' }) }));
vi.mock('../../../../hooks/useLocaleSettings', () => ({ useLocaleSettings: () => ({ numberLocale: 'en-US', dateLocale: 'en-US' }) }));
vi.mock('../../../../lib/notifyError', () => ({ notifyError: fixture.notifyError, logError: vi.fn() }));
vi.mock('../../../../lib/toast', () => ({ toast: fixture.toast }));
vi.mock('../../CollaborationPresence', () => ({ CollaborationPresence: () => null }));
vi.mock('../../PageHistory', () => ({ default: () => null }));
vi.mock('../../PageViewModal', () => ({ PageViewModal: (props: PageViewModalProps) => {
  useLayoutEffect(() => { pageViewClose = props.onClose; });
  return null;
} }));
vi.mock('../../IconPicker', () => ({ IconPicker: () => null }));
vi.mock('../../CoverPicker', () => ({ CoverPicker: () => null }));
vi.mock('../../MetadataLookupModal', () => ({ MetadataLookupModal: () => null }));
vi.mock('../../InsertContentModal', () => ({ InsertContentModal: () => null }));

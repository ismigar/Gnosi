...
import { useTableViewData } from './sharedCompatibility';
...
export function useTableData({ activeView, searchTerm, datedNotes, schema, safeNotes }: Inputs) {
  ...
  const { sortedPages: sortedAndFilteredNotes } = useTableViewData({ pages: datedNotes, schema, view: viewConfig, searchTerm });

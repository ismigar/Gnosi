...
export default function ModelRegistrySettings() {
    ...
    // Deleting a provider (in the sibling Providers section) cascades into
    // this registry server-side: reload so its rows disappear here too.
    useEffect(() => {
        const onModelsChanged = () => { load(); };
        window.addEventListener('gnosi-ai-models-changed', onModelsChanged);
        return () => window.removeEventListener('gnosi-ai-models-changed', onModelsChanged);
    }, [load]);

...
const metricNames = ['startupEntryBytes', 'largestChunkBytes', 'editorVendorBytes', 'tldrawVendorBytes', 'settingsRouteBytes'] as const;
...
export const BUNDLE_LIMITS: Readonly<BundleMetrics> = Object.freeze({
  startupEntryBytes: 1_400_000,
  largestChunkBytes: 1_800_000,
  editorVendorBytes: 1_550_000,
  tldrawVendorBytes: 1_350_000,
...
export function inspectBundle(dist: string, limits: Readonly<BundleMetrics> = BUNDLE_LIMITS): BundleMetrics {
  ...
  const modulePath = new URL(moduleSource, 'https://build.invalid/').pathname;
  const entry = /\/assets\/([a-zA-Z0-9_.-]+[.]js)$/u.exec(modulePath)?.[1];
  ...
  const metrics: BundleMetrics = {
    ...
    editorVendorBytes: checkedMaximum(chunks, 'editor-vendor-').bytes,
    tldrawVendorBytes: checkedMaximum(chunks, 'tldraw-vendor-').bytes,

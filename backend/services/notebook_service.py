...
def resolve_chat_contexts(
    ...
    *,
    ...
    for raw in requested:
        ...
        client_scope = raw.get("scope") if isinstance(raw.get("scope"), dict) else {}
        selection = str(client_scope.get("selection") or "all").lower()
        requested_source_ids = (
            client_scope.get("source_ids") if selection == "sources" else None
        )
...
def inspect_notebook(
    ...
    *,
    ...
    selected_source_ids = _authorized_source_ids(
        notebook_id, resolved_revision, source_ids
    )
    source_clause = ""
    ...
    if selected_source_ids is not None:
        ...
        source_clause = (
            " AND s.source_id IN ("
            + ",".join("?" for _item in selected_source_ids)
            + ")"
        )
        ...
        if match:
            try:
                candidates = connection.execute(
                    ...
                    (match, notebook_id, resolved_revision, *source_params),
                ).fetchall()
...
def start_notebook_analysis(
    ...
    *,
    ...
    selected_source_ids = _authorized_source_ids(
        notebook_id, int(revision), source_ids
    )
    if selected_source_ids == []:

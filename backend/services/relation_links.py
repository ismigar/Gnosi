...
def is_relation_key(key: object, relation_keys: set[str] | None = None) -> bool:
...
def relation_keys_from_table(table: object) -> set[str]:
    ...
    if is_record(table):
        for p in iterable_values(table.get("properties") or []):
            if is_record(p) and p.get("type") == "relation":
                name = p.get("name")
...
@overload
def strip_item(value: object) -> object: ...
...
def strip_relation_wikilinks(
    metadata: MetadataValue, relation_keys: set[str] | None = None
) -> MetadataValue:
    ...
    for key in record:
        ...
        if isinstance(value, list):
            record[key] = [strip_item(v) for v in value]
        else:
            record[key] = strip_item(value)
    return metadata
...
def _decorate_item(
    ...
    if decorated:
        ...
        if title_only:
            ...
            resolved_id = title_to_id(title_only.group("title")) if title_to_id else None
            if not resolved_id:
                return value
            rid = resolved_id
        else:
...
def decorate_relation_wikilinks(
    ...
    for key in record:
        ...
        value = record[key]
        if isinstance(value, list):
            record[key] = [_decorate_item(v, id_to_title, title_to_id) for v in value]
        else:
            record[key] = _decorate_item(value, id_to_title, title_to_id)
    return metadata

...
class PathResolver:
    ...
    def update_index(
        ...
        log.info(
            f"🚀 PathResolver updated for {v_str}: {len(id_to_path)} IDs, {len(all_files)} files."
        )
    ...
    def remove_file(self, vault_path: Path, record_id: str | None, file_path: Path | None) -> None:

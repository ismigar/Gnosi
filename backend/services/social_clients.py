...
class MastodonClient:
    ...
    async def get_home_timeline(self, limit: int = 20) -> list[JsonObject]:
    ...
    async def _get_trends(self, limit: int = 20) -> list[JsonObject]:
    ...
    async def get_notifications(self, limit: int = 20) -> list[JsonObject]:
    ...
    def _transform_posts(self, posts: list[JsonObject]) -> list[JsonObject]:
            ...
            transformed.append(
                {
                    ...
                    "reblog_by": post.get("account", {}).get("display_name")
                    if post.get("reblog")
                    else None,
    ...
    async def _upload_media(self, media: MediaItems | None) -> list[str]:
    ...
    async def post_status(
        self, status: str, media_ids: list[str] | None = None
    ) -> JsonObject | None:
    ...
    async def publish(self, text: str, media: MediaItems | None = None) -> JsonObject:
...
class BlueskyClient:
    ...
    async def get_timeline(self, limit: int = 20) -> list[JsonObject]:
    ...
    async def _get_whats_hot(self, limit: int = 20) -> list[JsonObject]:
    ...
    async def _upload_blob(self, path: str) -> JsonObject:
        ...
        async with httpx.AsyncClient() as client:
            ...
            resp.raise_for_status()
            return cast(JsonObject, resp.json()["blob"])

    async def create_post(self, text: str, media: MediaItems | None = None) -> JsonObject | None:
    ...
    def _transform_posts(self, feed: list[JsonObject]) -> list[JsonObject]:
        ...
        for item in feed:
            ...
            if (
                ...
                is_repost = True
                repost_by = reason.get("by", {}).get("displayName")
            else:
                is_repost = False
                repost_by = None
        ...
        async with httpx.AsyncClient() as client:
            if media:
                ...
                with open(path, "rb") as fh:
                    ...
                    resp = await client.post(
                        f"{self.base_url}/sendPhoto", data=data, files=files, timeout=60.0
                    )
            else:
                resp = await client.post(

...
def _sentence_case_term(term_lc: str, wanted: str) -> re.Pattern:
    """
        Builds a pattern that only matches the term at:
...
def normalize_text(s: str) -> str:
    ...
    for term_lc, wanted in SENTENCE_CASE_TERMS.items():
        ...
        def repl(m: re.Match) -> str:
            ...
            # If the match already has “correct” capitalization (e.g. fully capitalized in a title),
            # you can choose to respect it. Here we force the 'wanted' form only at the start of a sentence.

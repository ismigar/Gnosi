...
def _store() -> PlanningStore:
    ...
    return PlanningStore(Path(vault_path) / ".gnosi")
...
def _index() -> ScheduleIndex:
    ...
    if vault_path is None:
        raise HTTPException(status_code=503, detail="No active vault")
    return ScheduleIndex(Path(vault_path))
...
async def recalculate_project(project_id: str, payload: RecalculatePayload) -> Any:
    ...
    schedule = await asyncio.to_thread(
        build_schedule,
        [_payload(task) for task in payload.tasks],
        calendar,
...
async def create_baseline(project_id: str, payload: BaselinePayload) -> Any:
    ...
    if not schedule:
        raise HTTPException(
            status_code=409, detail="Recalculate the project before creating a baseline"
        )
    if payload.schedule_revision is not None and payload.schedule_revision != schedule.get(
        "scheduleRevision"
    ):
...
async def get_baseline_variance(project_id: str, baseline_id: str) -> Any:
    ...
    baseline = next(
        (
            ...
            if item.get("id") == baseline_id and item.get("projectId") == project_id
        ),
        None,
    )
    if not baseline:
    ...
    if not current:
        raise HTTPException(
            status_code=409, detail="Recalculate the project before requesting variance"
        )
    ...
    for task_id in sorted(set(original_tasks) | set(current_tasks)):
        ...
        rows.append(
            {
                "taskId": task_id,
                "baselineStart": original.get("start"),
                "currentStart": revised.get("start"),
                "baselineEnd": original.get("end"),
                "currentEnd": revised.get("end"),
                "durationDaysVariance": round(
                    float(revised.get("durationDays") or 0)
                    - float(original.get("durationDays") or 0),
                    4,
                ),
                "workHoursVariance": round(
                    float(current_costs.get(task_id, {}).get("planned_work_hours") or 0)
                    - float(baseline_costs.get(task_id, {}).get("planned_work_hours") or 0),
                    4,
                ),
                "costVariance": round(
                    float(current_costs.get(task_id, {}).get("estimated_cost") or 0)
                    - float(baseline_costs.get(task_id, {}).get("estimated_cost") or 0),
                    2,
                ),
            }
        )
    return {
        ...
        "baselineScheduleRevision": baseline.get("scheduleRevision"),
        "currentScheduleRevision": current.get("scheduleRevision"),
        "totalCostVariance": round(
            current_allocation["total_estimated_cost"]
            - float(baseline.get("allocation", {}).get("total_estimated_cost") or 0),
            2,
        ),
        "tasks": rows,
    }
    ...
    proposal.update(
        {
            "id": str(uuid.uuid4()),
            "projectId": project_id,
            "scheduleRevision": (schedule or {}).get("scheduleRevision", 0),
            "createdAt": datetime.now().isoformat(timespec="seconds"),
        }
    )
    proposal["type"] = "leveling_proposal"
    proposal["status"] = "pending"
    proposal["sourceEtags"] = {
        item["id"]: item.get("sourceEtag")
...
async def apply_leveling_proposal(proposal_id: str, payload: ProposalApplyPayload) -> Any:
    ...
    if payload.schedule_revision != proposal.get("scheduleRevision"):
        raise HTTPException(
            status_code=409, detail="Schedule revision is stale; regenerate the proposal"
        )
    schedule = ((await asyncio.to_thread(_index().load) or {}).get("projects") or {}).get(
        proposal["projectId"]
    )
    ...
    for change in proposal["proposals"]:
        ...
        if not assignment:
            raise HTTPException(
                status_code=409, detail="An assignment in the proposal no longer exists"
            )
        if assignment.get("start") != change.get(
            "source_start", assignment.get("start")
        ) or assignment.get("end") != change.get("source_end", assignment.get("end")):
            raise HTTPException(
                status_code=409, detail="An assignment changed; regenerate the proposal"
            )
...
async def materialize_recurrence(recurrence_id: str, limit: int = 50) -> Any:
    ...
    find_page_path_typed = cast(Callable[[str], Path | None], find_page_path)
    parse_frontmatter_typed = cast(
        Callable[[str, Path], tuple[dict[str, Any], str]], parse_frontmatter
    )
    save_page_typed = cast(Callable[..., Any], save_page_md)
    source_path: Path | None = await asyncio.to_thread(find_page_path_typed, recurrence["task_id"])
    if not source_path:
    ...
    try:
        ...
        occurrences = list(islice(rule, limit))
    ...
    for occurrence in occurrences:
        ...
        await asyncio.to_thread(save_page_typed, target, copy, content)
...
async def update_calendar(calendar_id: str, payload: CalendarPayload) -> Any:
    async with _mutation_lock:
        ...
        try:
            ...
            raise _validation_error(error) from error
        state["calendars"] = [
            calendar if item["id"] == calendar_id else item for item in state["calendars"]
        ]
        state = await asyncio.to_thread(store.save, state)
    return {"calendar": calendar, "revision": state["revision"]}
...
async def update_resource(resource_id: str, payload: ResourcePayload) -> Any:
    async with _mutation_lock:
        ...
        try:
            ...
            raise _validation_error(error) from error
        state["resources"] = [
            resource if item["id"] == resource_id else item for item in state["resources"]
        ]
        state = await asyncio.to_thread(store.save, state)
    return {"resource": resource, "revision": state["revision"]}
...
async def delete_resource(resource_id: str) -> Any:
    async with _mutation_lock:
        ...
        if any(item["resource_id"] == resource_id for item in state["assignments"]):
            raise HTTPException(
                status_code=409, detail="Delete resource assignments before deleting the resource"
            )
...
async def update_assignment(assignment_id: str, payload: AssignmentPayload) -> Any:
    async with _mutation_lock:
        ...
        try:
            ...
            raise _validation_error(error) from error
        state["assignments"] = [
            assignment if item["id"] == assignment_id else item for item in state["assignments"]
        ]
        state = await asyncio.to_thread(store.save, state)
    return {"assignment": assignment, "revision": state["revision"]}

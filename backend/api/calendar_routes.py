...
from backend.domains.calendar.timing import (
    calendar_phase,
    calendar_to_thread,
    collect_calendar_timing,
)
...
async def get_calendars(
    ...
    def read_integrations() -> dict[str, Any]:
        with calendar_phase("integrations"):
            return integration_manager.get_all_safe()

    # Request/Response remain injectable without becoming OpenAPI parameters;
    # the None fallback preserves existing direct internal/test calls.
    with collect_calendar_timing(
        request is not None and request.headers.get("x-gnosi-calendar-timing") == "1"
    ) as timing:
        integrations = await calendar_to_thread(read_integrations)
        email_list = calendar_runtime.account_emails(integrations, email)
        calendars, auth_errors = await calendar_to_thread(
...
async def get_events(
    ...
    with collect_calendar_timing(
        request is not None and request.headers.get("x-gnosi-calendar-timing") == "1",
        profile=request is not None and request.headers.get("x-gnosi-calendar-profile") == "1",
    ) as timing:
        events = await calendar_to_thread(
            collect_all_events, time_min, time_max, search, calendar_id, include_vault, email
        )
    if timing is not None and response is not None:
        response.headers["Server-Timing"] = timing.header()
        if timing.profile_id is not None:
            response.headers["X-Gnosi-Request-Profile-Id"] = timing.profile_id
    return events
...
async def get_meeting_reminders() -> dict[str, Any]:
    ...
    return {"reminders": await asyncio.to_thread(get_active)}
...
async def get_event(
    ...
    ev = await asyncio.to_thread(_get_event, email, event_id, calendar_id)
    if ev:
        return ev

...
class _Subscriber:
    ...
    def __init__(self: Any, account_filter: Optional[str] | None = None) -> None:
...
class ImapIdleManager:
    ...
    def subscribe(self: Any, account_filter: Optional[str] | None = None) -> _Subscriber:
    ...
    def _broadcast(self: Any, event: dict[str, Any]) -> None:
        with self._sub_lock:
    ...
    def _broadcast_idle_line(self: Any, email_account: str, raw_line: str) -> None:
        event_type = None
        if " EXISTS" in raw_line:
            event_type = "new_message"
            log.info(f"[IDLE] {email_account}: new message ({raw_line})")
        elif " EXPUNGE" in raw_line:
            event_type = "message_removed"
        elif " FETCH" in raw_line:
            event_type = "flags_changed"
        if event_type:
            self._broadcast({"account": email_account, "type": event_type, "raw": raw_line})

    def _read_idle_lines(
        self: Any,
        imap: Any,
        email_account: str,
        stop: threading.Event,
        ...
        while not stop.is_set() and time.monotonic() - started_at <= _IDLE_REFRESH_S:
            try:
                line = imap.readline()
            except socket.timeout:
                continue
            except OSError as exc:
                log.debug(f"[IDLE] Socket OSError per {email_account}: {exc}")
                return
            if not line:
                return
            self._broadcast_idle_line(email_account, line.decode("utf-8", errors="replace").strip())

    def _idle_session(self: Any, imap: Any, email_account: str, stop: threading.Event) -> None:
        ...
        try:
            ...
            self._read_idle_lines(imap, email_account, stop, start)
        finally:
            # DONE to exit IDLE
            try:
                sock.settimeout(10)

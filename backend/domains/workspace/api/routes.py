...
async def create_workspace(
    ...
    # 3. Create Workspace
    new_ws = Workspace(name=request.name, slug=slug)
    db.add(new_ws)
    db.flush()  # To get the ID

    # 4. Assign creator as OWNER
    membership = Membership(user_id=x_user_id, workspace_id=new_ws.id, role=UserRole.OWNER.value)
    db.add(membership)
...
async def list_workspaces(
    ...
    memberships = db.query(Membership).filter(Membership.user_id == x_user_id).all()
...
async def update_member_role(
    ...
    membership = (
        db.query(Membership)
        .filter(Membership.workspace_id == workspace_id, Membership.user_id == target_user_id)
        .first()
    )
...
async def add_workspace_member(
    ...
    if existing:
        raise HTTPException(
            status_code=400, detail="The user is already a member of this workspace"
        )
...
async def get_workspace(
    ...
    membership = (
        db.query(Membership)
        .filter(Membership.user_id == x_user_id, Membership.workspace_id == workspace_id)
        .first()
    )

    if not membership:
        raise HTTPException(
            status_code=403, detail="You do not have permission to access this workspace"
        )
...
async def list_member_vault_access(
    ...
    access_list = (
        ...
        .filter(VaultAccess.workspace_id == workspace_id, VaultAccess.user_id == user_id)
        .all()
    )
...
async def grant_vault_access(
    ...
    vault = (
        db.query(Vault)
        .filter(Vault.id == request.vault_id, Vault.workspace_id == workspace_id)
        .first()
    )
    if not vault:
    ...
    existing = (
        db.query(VaultAccess)
        .filter(VaultAccess.vault_id == request.vault_id, VaultAccess.user_id == user_id)
        .first()
    )
...
async def revoke_vault_access(
    ...
    access = (
        db.query(VaultAccess)
        .filter(VaultAccess.vault_id == vault_id, VaultAccess.user_id == user_id)
        .first()
    )

...
/**
 * Handles loading and saving Notion Bridge configuration.
 *
 * This class abstracts access to the `notion_bridge.settings` config object,
...
class ConfigManager {
  ...
  public function save(array $data): void {
    $this->configFactory
      ->getEditable('notion_bridge.settings')
  ...
  public function getCacheTtl(): int {
    $ttl = $this->configFactory->get('notion_bridge.settings')->get('cache_ttl');
  ...
  /**
   * Retrieves the maximum number of retry attempts for Notion API calls.
   *
   * @return int
   *   The number of retries. Defaults to 3 if not configured.
   */
  public function getMaxRetries(): int {
    $val = $this->configFactory->get('notion_bridge.settings')->get('max_retries');
    return $val !== null ? (int) $val : self::MAX_RETRIES;
  }

  /**
   * Sets the maximum number of retry attempts for Notion API calls.
   *
   * @param int $value
  ...
  public function getTimeFullSync(): int {
    $val = $this->configFactory->get('notion_bridge.settings')->get('time_full_sync');
  ...
  /**
   * Retrieves the timeout for Notion API requests in seconds.
   *
   * @return int
   *   Timeout duration in seconds. Defaults to 60 if not configured.
   */
  public function getTimeout(): int {
    $val = $this->configFactory->get('notion_bridge.settings')->get('timeout');
    return $val !== null ? (int) $val : 60;
  }

  /**
   * Sets the timeout for Notion API requests.
   *
   * @param int $value

...
/**
 * Configuration form for the Notion Bridge module.
 *
 * Allows administrators to configure API connection and database mapping
 * between Notion and Drupal content types.
 */
class NotionBridgeConfigForm extends FormBase {

  /**
   * The Notion client service.
   *
   * Handles API interaction with Notion.
   *
   * @var \Drupal\notion_bridge\NotionClient
   */
  ...
  /**
   * The configuration manager for the Notion bridge.
   *
   ...
   *   The configuration manager for Notion Bridge.
   */
  ...
  public static function create(ContainerInterface $container): static {
    return new static(
      ...
      $container->get('notion_bridge.config_manager')
    );
  }
  ...
  public function getFormId(): string {
    return 'notion_bridge_settings_form';
  }

  /**
   * Builds the configuration form for the Notion Bridge module.
   *
   * Allows the user to input an API key, load Notion databases, and map
  ...
  public function buildForm(array $form, FormStateInterface $form_state): array {
    $api_key = $this->notionClient->getNotionApiKey();
    ...
    $form['#prefix'] = '<div id="notion-bridge-wrapper">';
    $form['#suffix'] = '</div>';

    if (!empty($api_key)) {
      try {
        $databases = $this->notionClient->searchDatabases();
  ...
  private function buildApiKeyField(array $form, string $api_key): array {
    ...
    $form['api_key_wrapper']['help'] = [
      '#type' => 'markup',
      '#markup' => $this->t(
        '<p><strong>Note:</strong> The Notion API Key is not stored in the Drupal configuration database for security reasons.</p>
         ...
         <pre>$settings[\'notion_api_key\'] = \'your-secret-key-here\';</pre>
    ...
    $form['api_key_wrapper']['api_key'] = [
      '#type' => 'textfield',
      '#title' => $this->t('Notion API Key'),
      '#default_value' => $this->notionClient->getNotionApiKey() ?: '',
      '#attributes' => [
        'id' => 'edit-api-key',
        'type' => 'password',
        'autocomplete' => 'off',
    ...
    $form['#attached']['library'][] = 'notion_bridge/show_password';
  ...
  /**
   * Builds the form section for a specific Notion database.
   *
   * @param array $form
   *   The existing form array.
   ...
   * @param array $db
   *   The Notion database information (id and name).
   * @param array $saved
   ...
   *
   * @param string $dbId
   *   The Notion database ID associated with this select.
  ...
  /**
   * Builds a select form element for choosing the Notion publication field.
   *
   * This field is used to determine whether a Notion entry should be published.
   *
   * @param string $dbId
   *   The ID of the Notion database.
   * @param string $dbName
   *   The name of the Notion database (for user-facing messages).
  ...
  private function buildPublishPropertySelect(string $dbId, string $dbName, array $existing): array {
    $options = [];
    try {
      $notion_fields = $this->loadNotionFields($dbId);
      foreach ($notion_fields as $field) {
        $label = $field['name'];
        $options[$label] = $label;
      }
    }
      ...
      $this->messenger()->addWarning($this->t(
        'Failed to load fields from Notion database "@name": @msg',
    ...
    return [
      '#type' => 'select',
      '#title' => $this->t('Notion field to indicate publication'),
      '#options' => $options,
      '#default_value' => $existing['publish_property'] ?? NULL,
      '#empty_option' => $this->t('- None -'),
      '#description' => $this->t('Select the Notion field that indicates whether the record should be published (e.g., a "Publish" checkbox).'),
    ];
  }

  /**
   * Builds the field mapping section between Notion and Drupal fields.
   *
   * @param string $dbId
   *   The ID of the Notion database.
  ...
  private function buildFieldMappingSection(string $dbId, ?string $selected_type, array $saved_mapping): array {
    $section = [
      '#type' => 'details',
      '#title' => $this->t('Field mapping Notion → Drupal'),
      '#collapsible' => TRUE,
      '#open' => TRUE,
    ];
  ...
  /**
   * Adds global Notion API configuration settings to the form.
   *
  ...
  protected function buildGlobalSettings(array $form): array {
    ...
    $form['global_settings']['max_retries'] = [
      ...
      '#min' => 0,
      '#description' => $this->t('Number of retry attempts when Notion API returns rate limits (HTTP 429). Default: 3.'),
    ];
    ...
    $form['global_settings']['time_full_sync'] = [
      ...
      $current_batch = (int) (\Drupal::config('notion_bridge.settings')->get('sync.batch_size') ?? 200);
      $form['global_settings']['sync_batch_size'] = [
          '#type' => 'number',
          '#title' => $this->t('Sync flush batch size'),
          '#default_value' => $current_batch > 0 ? $current_batch : 200,
          '#min' => 1,
          '#description' => $this->t('How many content-key groups to process before flushing pending writes to Notion. Default: 200.'),
      ];
  ...
  /**
   * Loads and sorts Notion fields for the given database.
   *
   * @param string $dbId
   *   The Notion database ID.
   *
   * @return array
   *   A list of Notion field definitions.
   */
  private function loadNotionFields(string $dbId): array {
    try {
      $raw = $this->notionClient->getDatabaseProperties($dbId);
   ...
   * for rebuilding, which will re-trigger the loading of Notion databases.
   *
   * @param array $form
   *   The form structure.
   * @param \Drupal\Core\Form\FormStateInterface $form_state
  ...
  /**
   * Form submission handler for the Notion Bridge configuration form.
   *
   * This processes the user input, collects the enabled databases,
   * maps Notion properties to Drupal fields, and saves the configuration.
   *
   * @param array $form
  ...
  public function submitForm(array &$form, FormStateInterface $form_state): void {
    ...
    foreach ($databases as $db) {
      ...
      if (!empty($entry['field_mapping'])) {
        foreach ($entry['field_mapping'] as $notionFieldMapKey => $drupalField) {
          $notionFieldName = str_replace('_map', '', $notionFieldMapKey);
          if (!empty($drupalField)) {
            $temp['field_mapping'][$notionFieldName] = $drupalField;
          }
        }
      }
      ...
      // Capture per-bundle field map and invert to Drupal→Notion for columns.field_map.<bundle>.
      if (!empty($temp['drupal_type']) && !empty($temp['field_mapping'])) {
        $inverted = [];
        foreach ($temp['field_mapping'] as $notionProp => $drupalField) {
          // Only keep valid, non-empty pairs.
          if (!empty($drupalField) && is_string($drupalField)) {
            $inverted[$drupalField] = $notionProp;
          }
        }
        if (!empty($inverted)) {
    ...
    $editable = \Drupal::configFactory()->getEditable('notion_bridge.settings');

    // databases.<bundle> → Notion DB ID
    if (!empty($dbMap)) {
      $editable->set('databases', $dbMap);
    }

    // columns.field_map.<bundle> → Drupal→Notion field maps (inverted)
    ...
    \Drupal::configFactory()->getEditable('notion_bridge.settings')
  ...
  /**
   * Builds the Translation settings subsection for a Notion database.
   *
   ...
   *   Notion database ID.
   * @param string $dbName
   *   Notion database name (for messages).
   * @param array $existing
  ...
  private function buildTranslationSettingsSection(string $dbId, string $dbName, array $existing): array {
    // Load Notion fields to populate selects.
    $options = [];
    try {
      $notion_fields = $this->loadNotionFields($dbId);
      foreach ($notion_fields as $field) {
    ...
    $section['columns']['content_key_base'] = [
      ...
      '#description' => $this->t('Base Notion property where Drupal writes the generated content key. Do not edit this manually in Notion.'),
    ];

    // Formula Content_key: derived field in Notion that resolves original vs translation.
    ...
    // Relation to the original entry (Notion relation property).
    $section['columns']['original_rel'] = [
      ...
      '#description' => $this->t('Select the Notion relation property that links a translation to its original entry.'),
    ];
    ...
    $section['columns']['lang'] = [
      ...
      '#empty_option' => $this->t('- Select -'),
      '#description' => $this->t('Select the Notion property that stores the language code (e.g., ca, en, es).'),
    ];
    ...
    $section['columns']['status'] = [
      ...
      '#empty_option' => $this->t('- Select -'),
      '#description' => $this->t('Select the Notion property used to indicate publication status.'),
    ];
    ...
    $section['columns']['title'] = [
      ...
      '#empty_option' => $this->t('- Select -'),
      '#description' => $this->t('Select the Notion property that holds the record title.'),
    ];

...
/**
 * Drush commands for the Notion Bridge module.
 *
 * Provides CLI utilities for syncing and cleaning Notion content in Drupal.
 */
final class NotionBridgeCommands extends DrushCommands {
  ...
  /**
   * Sync manager to pull Notion content into Drupal (nodes + translations).
   *
   * @var \Drupal\notion_bridge\NotionBridgeSyncManager
   */
  protected NotionBridgeSyncManager $syncManager;
  ...
  /**
   * Constructs the NotionBridgeCommands class.
   *
   * @param \Drupal\notion_bridge\Cleaner $cleaner
   *   Service to clean orphaned file entities.
   * @param \Drupal\notion_bridge\NotionBridgeSyncManager $sync_manager
   *   Sync manager to pull Notion content into Drupal (nodes + translations).
  ...
  public function __construct(
    Cleaner $cleaner,
    NotionBridgeSyncManager $sync_manager,
  ...
  /**
   * Shows the current sync status for each tracked Notion database.
   *
   * @command notion-bridge:status
   * @aliases nb-status
   */
  #[CLI\Command(
    name: 'notion-bridge:status',
    aliases: ['nb-status'])]
  #[CLI\Help(
    description: 'Shows the current sync status for each tracked Notion database.'
  )]
  public function status(): void {
    ...
    if (empty($dbMap)) {
      $io->warning('No bundles configured under notion_bridge.settings:databases.');
      return;
    }

    $io->title('Notion Bridge Status');
    foreach ($dbMap as $bundle => $dbId) {
      $ts = (int) $this->state->get("notion_bridge.last_sync.$bundle", 0);
  ...
  /**
   * Synchronizes Notion → Drupal for one bundle or all configured bundles.
   *
   * If a bundle is provided, syncs only that one. Otherwise, syncs all
   * bundles configured under notion_bridge.settings:databases.
   ...
   *
   * @command notion-bridge:sync
   * @aliases nb-sync
   * @option full Force full synchronization (ignore last sync timestamp).
   */
  #[CLI\Command(name: 'notion-bridge:sync', aliases: ['nb-sync'])]
  #[CLI\Help(description: 'Sync Notion databases into Drupal for one bundle or all configured bundles.')]
  ...
  public function sync(?string $bundle = NULL, array $options = ['full' => false]): void {
    ...
    if ($bundle) {
      if (!isset($dbMap[$bundle])) {
        $io->error("Bundle '$bundle' is not configured under notion_bridge.settings:databases.");
        return;
      }
      $bundles = [$bundle];
    }
    ...
    if (empty($bundles)) {
      $io->warning('No bundles to sync. Configure databases under notion_bridge.settings:databases.');
      return;
    }
    ...
    foreach ($bundles as $b) {
      ...
      try {
        ...
        $this->state->set("notion_bridge.last_sync.$b", time());
   ...
   *
   * @command notion-bridge:cleanOrphans
   * @aliases nb-clean
   */
  #[CLI\Command(
    name: 'notion-bridge:cleanOrphans',
    aliases: ['nb-clean']
  )]
  #[CLI\Help(

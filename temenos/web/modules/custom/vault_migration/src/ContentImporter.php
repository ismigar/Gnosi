...
/**
 * [LEGACY] Service class that imports Vault pages into Drupal nodes.
 *
 * Aquest component s'utilitza exclusivament per a la migració de dades històriques
 * de Vault cap al graf de coneixement de Gnosi. El funcionament diari de l'app
 * ja és independent d'aquest importador.
 *
...
class ContentImporter {
  ...
  /**
   * Handles storage and retrieval of Vault-to-Drupal mapping data.
   *
  ...
  /**
   * Pages that contain unresolved mentions to other Vault pages.
   *
   * Format:
   * [
   *   'source_nid' => [
   *     'body' => '... raw HTML with unresolved mentions ...',
   *     'mentioned_ids' => ['vault_id_1', 'vault_id_2']
   *   ],
   *   ...
   * ]
   *
   ...
   *   The mapping storage service for Vault-to-Drupal mappings.
  ...
  /**
   * Imports a Vault database by its ID and creates or updates Drupal nodes.
   *
  ...
  public function import(string $databaseId, ?int $lastSynced = null): void {
    ...
    $dbMeta = $this->vaultClient->retrieveDatabase($databaseId);

    $propMeta = $dbMeta['properties'][$publish_field_key] ?? [];

    // 4. Build filter to only fetch published items.
    $finalFilter = $this->vaultClient->buildPublishFilter($propMeta, $publish_field_key);

    // 5. Query Vault
    $publishedIds = [];
    // queryDatabaseAll() retorna un \Generator – cal convertir-ho si vols ordenar.
    $pages_iter = $this->vaultClient->queryDatabaseAll($databaseId, $finalFilter);
  ...
  /**
   * Marks a Vault page mention for later resolution.
   *
   * This method is called during the first import pass when a Vault page
   * contains a reference to another Vault page whose Drupal alias is not
   * yet available. It stores the raw HTML and the unresolved Vault ID
   * for post-processing.
   *
   * @param string $mentioned_id
   *   The normalized Vault page ID that was mentioned in rich text.
   */
   ...
   *   The Drupal node ID.
   * @param string $html
   *   The HTML content to scan for <span data-vault-id="...">.
   */
  private function collectPendingMentionsFromHtml(int $nid, string $html): void {
    // Early return if $html is empty or doesn't contain data-vault-id.
    if (empty($html) || strpos($html, 'data-vault-id') === false) {
      return;
    }
    if (preg_match_all('/<span\s+data-vault-id="([^"]+)">.*?<\/span>/is', $html, $m)) {
      if (!empty($m[1])) {
        $this->pendingMentions[$nid] = [
          'body' => $html,
        ...
        $queue = $this->queueFactory->get('vault_migration_mentions');
        foreach ($ids as $id) {
          $queue->createItem([
            'nid' => $nid,
            'langcode' => $langcode,
  ...
  /**
   * Replaces pending Vault mentions with actual links in the imported nodes.
   *
   * During the first import pass, mentions to Vault pages whose aliases
   * are not yet known are stored as <span data-vault-id="...">...</span>.
  ...
  protected function resolvePendingMentions(): void {
    ...
    foreach ($this->pendingMentions as $nid => $data) {
      ...
      foreach (array_unique($data['mentioned_ids']) as $vault_id) {
        // 1) Troba nid per Vault ID i després l'àlies públic de /node/{nid}.
        if (!isset($aliasCache[$vault_id])) {
          $nid = $this->mappingStorage->getNodeIdByVaultId($vault_id);
          if ($nid) {
            $aliasCache[$vault_id] = \Drupal::service('path_alias.manager')->getAliasByPath('/node/' . $nid);
          } else {
            $aliasCache[$vault_id] = '';
          }
        }

        $alias = $aliasCache[$vault_id];
        if ($alias) {
          // Patró tolerant: permet altres atributs/ordre dins el <span ...>.
          $pattern = '/<span[^>]*\\sdata-vault-id="' . preg_quote($vault_id, '/') . '"[^>]*>(.*?)<\\/span>/is';
...
// $this->fileUsage->delete($file, 'vault_migration', 'node', $node->id());
// just abans de $file->delete();

  /**
   * Builds a Drupal batch for importing Vault pages.
   *
   ...
   *   List of Vault page IDs to import.
   * @param string $databaseId
  ...
  private function buildImportBatch(array $page_ids, string $databaseId, array $entry): array {
    $builder = (new \Drupal\Core\Batch\BatchBuilder())
      ->setTitle($this->t('Importing from Vault'))
  ...
  /**
   * Removes Drupal nodes that were previously synced but no longer published in Vault.
   *
   * @param array $publishedIds
   *   The list of published Vault page IDs returned in the current sync.
   * @param string $databaseId
   *   The Vault database ID.
   *
   * @result void
  ...
  private function cleanupUnpublished(array $publishedIds, string $databaseId): void {
    $allMapped = array_map(
      [$this->mappingStorage, 'normalizeVaultId'],
      $this->mappingStorage->getVaultIdsByDatabase($databaseId)
    );

    $publishedIds = array_map([$this->mappingStorage, 'normalizeVaultId'], $publishedIds);
    ...
    if ($totalPublished === 0 && $totalMapped > 0) {
      $this->logger->error(
        '⚠️ Sync aborted: Vault returned 0 published entries, but @n_local nodes are mapped.',
        ['@n_local' => $totalMapped]
      );
      return;
    }
    ...
    if ($totalToDelete > self::MAX_DELETE_LIMIT) {
      $this->logger->warning(
        '⚠️ Sync aborted: detected @n_delete nodes to delete (limit = @max). Check Vault status before retrying.',
    ...
    foreach ($toDelete as $pageId) {
      ...
      if ($nid = $this->mappingStorage->getNodeIdByVaultId($pageId)) {
  ...
  public static function batchFinished($success, $results, $operations) {
    if (!$success) {
      \Drupal::logger('vault_migration')->error('❌ Vault import batch failed.');
      return;
    }

    /** @var \Drupal\vault_migration\ContentImporter $importer */
    $importer = \Drupal::service('vault_migration.content_importer');
  ...
  /**
   * Batch callback to process a single Vault page with its config entry.
   *
   * @param array $page_id
   *   The Vault id page to import.
   * @param array $entry
  ...
  public static function runImportPageBatch(string $page_id, array $entry, string $database_id, array &$context): void {
    \Drupal::service('vault_migration.page_batch_handler')->run($page_id, $entry, $database_id, $context);
  }


  /**
   * Imports or updates a single Vault page into a Drupal node.
   *
   * @param array $item
   *   The Vault page data.
   * @param array $entry
   ...
   * @param string $databaseId
   *   The Vault database ID (used for mapping storage).
   * @param array &$publishedIds
  ...
  public function processSinglePage(array $item, array $entry, string $databaseId, array &$publishedIds): void {
    try {
      ...
      $pageId = $this->mappingStorage->normalizeVaultId($item['id']);
      $item = $this->vaultClient->getPage($pageId);

      // DEBUG segur: la propietat de fitxers a Vault és 'files' (array). No totes les pàgines la tindran.
      ...
      if (!$this->vaultClient->isPublished($item, $publishFieldKey)) {
        return;
      }
      ...
      $title   = $titleField ? $this->vaultClient->extractPlainText($item['properties'][$titleField] ?? []) : 'Untitled';
      $langcode = $this->vaultClient->getLangcode($item, $mapping);
      $created  = $this->vaultClient->getCreatedDate($item, $mapping);

      // Render body HTML from Vault (pot fallar; millor capturar excepcions si és sensible).
      $bodyHtml = $this->vaultClient->renderVaultPageAsHtml($pageId);
      ...
      $existing_nid = $this->mappingStorage->getNodeIdByVaultId($pageId);
      ...
      // Collect unresolved mentions (<span data-vault-id="...">) for later resolution.
  ...
  /**
   * Determines whether a Vault page has been modified since the last sync.
   *
   * @param string $pageId
   *   Vault page ID.
   * @param string $databaseId
   ...
   *   Timestamp of the last edit on the Vault page.
   *
   * @return bool
  ...
  /**
   * Maps Vault properties to corresponding Drupal node fields.
   *
   * @param \Drupal\node\Entity\Node $node
   *   The Drupal node to populate.
   * @param array $mapping
   *   Array of Vault property => Drupal field mappings.
   * @param array $item
   *   The Vault page properties array.
  ...
  private function mapFieldsToNode(Node $node, array $mapping, array $item, string $drupalType, string $pageId): void {
    foreach ($mapping as $vaultField => $drupalField) {
      if (in_array($drupalField, ['title', 'langcode', 'uid', 'created'])) {
        continue;
      }

      $prop = $item['properties'][$vaultField] ?? null;
      if (!$prop) continue;
      ...
      switch ($fieldType) {
        ...
        case 'file':
          if (!empty($prop['files'])) {
            if ($fieldType === 'image') {
              $this->importImages($node, $drupalField, $prop, $pageId, $vaultField);
            } else {
              $this->importFiles($node, $drupalField, $prop, $pageId, $vaultField);
            }
          }
          break;
        ...
        case 'text_long':
          $node->set($drupalField, [
            'value' => $this->vaultClient->extractRichText($prop),
            'format' => 'full_html',
          ]);
          break;
        case 'text':
          $node->set($drupalField, $this->vaultClient->extractPlainText($prop));
          break;
        case 'string':
        case 'string_long':
          $node->set($drupalField, $this->vaultClient->extractPlainText($prop) ?: NULL );
          break;
        case 'email':
        case 'url':
        case 'rich_text':
        ...
        $this->logger->debug(
          'Map link: @field Vault shape=@shape => @value',
          [
            '@field' => $drupalField,
        ...
        break;
        case 'datetime':
          if ($ts = $this->vaultClient->extractDateStart($prop)) {
        ...
        case 'entity_reference':
          ...
          if ($target_type === 'taxonomy_term') {
            ...
            if ($fid = $this->vaultClient->handleImageOrFile($prop)) {
   ...
   * from a Vault property that may contain a URL (directly or nested).
   *
  ...
  protected function buildUrlLikeValue(\Drupal\node\NodeInterface $node, string $drupalField, mixed $prop): mixed {
    ...
    $url = $this->vaultClient->findFirstUrlInVaultProperty($prop);
  ...
  /**
   * Finds a config entry by Vault database ID.
   *
   * @param array $entries
  ...
  private function findEntryById(array $entries, string $id): ?array {
    $id = $this->mappingStorage->normalizeVaultId($id);
    foreach ($entries as $entry) {
      if (strcasecmp($this->mappingStorage->normalizeVaultId($entry['id']), $id) === 0) {
        return $entry;
      }
    }
    return null;
  ...
  /**
   * Ensures a given Vault select value exists in a list_string field.
   *
   * @param string $vaultLabel
   *   Human-readable value from Vault.
   * @param string $drupalField
   ...
   */
  protected function normalizeSelectValue(string $vaultLabel, string $drupalField, string $drupalType): ?string {
    $machineKey = mb_strtolower(trim($vaultLabel), 'UTF-8');
    ...
    $allowedValues[$machineKey] = $vaultLabel;
  ...
  /**
   * Logs Vault API query exceptions with contextual details.
   *
   * @param \Exception $e
   *   The exception thrown by the API call.
   * @param string $dbName
   *   Human-readable Vault database name.
   */
  ...
  /**
   * Imports Vault files/external links into a Drupal file field.
   **
   ...
   *   Vault property array (expects ['files' => [...]]).
   * @param string $pageId
   *   Vault page ID (for refreshing presigned URLs).
   * @param string $vaultField
   *   Vault field name to refresh.
   */
  private function importFiles(NodeInterface $node, string $field_name, array $property, string $pageId, string $vaultField): void {
    if (!$node->hasField($field_name) || empty($property['files'])) {
      $this->logger->error('Field @name missing on node or Vault property is empty.', ['@name' => $field_name]);
      return;
    }
    ...
    $refreshProperty = function(array $fallback) use ($pageId, $vaultField): array {
      try {
        $fresh = $this->vaultClient->getPage($pageId);
        return $fresh['properties'][$vaultField] ?? $fallback;
      }
      catch (\Throwable $e) {
        return $fallback;
      }
    ...
    foreach ($items as $delta => $item) {
      ...
      try {
        ...
        $this->fileUsage->add($file, 'vault_migration', 'node', (int) $node->id());

        /**
         * Build FileItem array so it renders on view:
         * - 'display' => 1 makes the Generic file formatter show the item.
         * - 'description' uses Vault's filename (optional).
         */
        $itemValue = [
          'target_id'  => $file->id(),
          'display'    => 1,
  ...
  /**
   * Imports Vault files/external images into a Drupal image field.
   *
   * @param \Drupal\node\NodeInterface $node
   *   Target node.
   * @param string $field_name
   ...
   *   Vault field name to refresh.
   */
  private function importImages(NodeInterface $node, string $field_name, array $property, string $pageId, string $vaultField): void {
    if (!$node->hasField($field_name) || empty($property['files'])) {
      $this->logger->error('Field @name missing on node or Vault property is empty.', ['@name' => $field_name]);
      return;
    }
    ...
    // Helper for refreshing Vault property when presigned URLs expire.
    $refreshProperty = function(array $fallback) use ($pageId, $vaultField): array {
      try {
        $fresh = $this->vaultClient->getPage($pageId);
        return $fresh['properties'][$vaultField] ?? $fallback;
   ...
   * @param string $uri
   *   Destination URI (e.g. public://vault_files/foo.pdf).
   * @param string $filename
  ...
  /**
   * Prepares a list of batch operations to import all pages from a Vault database.
   *
   * @param string $database_id
   *   The ID of the Vault database to process. Must not be empty.
   * @param int|null $last_synced
   *   Optional timestamp of the last synchronization. If provided, only pages
   *   modified after this time will be included via a Vault filter.
   *
   * @return array
   ...
   * @throws \RuntimeException
   *   If the Vault request fails or yields invalid data.
   */
  public function prepareImportBatch(string $database_id, array $entry, ?int $last_synced = NULL): array {
    ...
    // Query the Vault database with optional filter for recently modified pages.
    $finalFilter = $this->buildFilter($last_synced);

    $page_ids = [];
    foreach ($this->vaultClient->queryDatabaseAll($database_id, $finalFilter) as $page) {
      $page_ids[] = $page['id'] ?? null;
    }
    $page_ids = array_filter($page_ids);
  ...
  /**
   * Builds a Vault API filter for pages modified after the last sync.
   *
   * @param int|null $last_synced
   *   The timestamp of the last successful synchronization.
   *
   * @return array
   *   A Vault filter array or empty array if not needed.
   */

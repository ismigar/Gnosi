...
/**
 * Synchronizes Vault pages with Drupal nodes and their translations.
 *
 * Design goals:
 ...
 *   base property in Vault (never to rollups or formulas).
 * - Group Vault rows by content_key and upsert base node + per-language
 *   translations in an idempotent way.
 *
 * This class assumes the module exposes:
 *  - A Vault client service capable of fetching table rows and updating fields.
 *  - A MappingStorage to store additional metadata if needed (optional here).
 */
final class VaultMigrationSyncManager {
   ...
   * @param object $vaultClient
   *   The Vault client (existing service: vault_migration.vault_client).
   ...
   *   To read field names configured in admin (column names in Vault).
   */
  public function __construct(
    ...
    private readonly object $vaultClient,
  ...
  public function sync(string $bundle): void {
    ...
    // Column names in Vault (admin-configurable).
    ...
    $map = $this->kvFactory->get('vault_migration.content_map');

    // 1) Fetch Vault rows and group them by ROOT page (Original relation).
    ...
    $byRoot = [];
    $items = iterator_to_array($this->vaultClient->fetchRows($bundle));
    ...
    foreach ($items as $itRow) {
      ...
      elseif (!empty($itRow['_raw_props'][$colOriginalRel])) {
        $ids = $this->vaultClient->extractRelationIds($itRow['_raw_props'][$colOriginalRel]);
    ...
    foreach ($items as $row) {
      $rowId = (string) ($row['id'] ?? '');
      if ($rowId === '') {
        $this->logger->warning('Skipping Vault row without ID.');
        continue;
      }
    ...
    foreach ($byRoot as $rootId => $group) {
      ...
      if ($contentKey === '') {
        ...
        $this->logger->debug('No content_key found for root @root. Generating and writing @key to Vault.', [
          '@root' => $rootId,
          '@key' => $contentKey,
        ]);
        try {
          // Prefer a direct update for the root page so all rollups/formulas can recompute immediately.
          $this->vaultClient->updatePageProperties($rootId, [$colContentKeyBase => $contentKey]);
          ...
          $this->logger->error('Failed to write Content_key to Vault root @id: @msg', ['@id' => $rootId, '@msg' => $e->getMessage()]);
          // $this->vaultClient->queueUpdate($rootId, [$colContentKeyBase => $contentKey]);
        }
      }
      ...
      try {
        ...
        $this->vaultClient->updatePageProperties($rootId, [
        ...
        foreach (($group['rows'] ?? []) as $r) {
          $pid = (string) ($r['id'] ?? '');
          if ($pid && $pid !== $rootId) {
            $this->vaultClient->updatePageProperties($pid, [
      ...
      if ($batchSize > 0 && (++$processed % $batchSize) === 0) {
        $this->vaultClient->flush();
      }
    }

    // 3) Flush pending back-writes to Vault (e.g., base Content_key set).
    $this->vaultClient->flush();
  }

  /**
   * Extract a title string from Vault raw properties if possible.
   *
   * @param array $rawProps
   *   The raw Vault properties array for a page.
   *
   * @return string
  ...
  private function resolveEffectiveContentKey(
    ...
    if ($effective === '') {
      ...
      $this->vaultClient->queueUpdate($rowId, [$colBase => $effective]);
      $this->logger->info('Generated new content_key for Vault row @id', ['@id' => $rowId]);
    }
   ...
   *   Vault status column name.
   * @param string $colTitle
   *   Vault title column name.
   * @param string $statusPublishedValue
   *   Lowercased value in Vault that indicates a published translation.
   * @param array<string,string> $fieldMap
   *   Optional field mapping: [drupal_field_name => vault_property_name].
   */
  private function upsertGroup(
    ...
    foreach ($rows as $row) {
      ...
      // Example: publish/unpublish per language from Vault 'estat'.
      ...
      // Optional configurable field mapping: [drupal_field => vault_property].
      foreach ($fieldMap as $drupalField => $vaultProp) {
        // Prefer flattened value from the original Vault row snapshot.
        $val = $row['_raw'][$vaultProp] ?? NULL;
        // If still null and raw props exist, attempt to use the raw property as-is (some callers may handle arrays).
        if ($val === NULL && isset($row['_raw_props'][$vaultProp])) {
          $val = $row['_raw_props'][$vaultProp];
        }

        // Log when the Vault property is missing in this row.
        if ($val === NULL) {
        ...
        if ($translation->hasField($drupalField)) {
          try {
            ...
            $this->logger->error('Failed to set field "@field" from Vault property "@prop": @msg', [
              '@field' => $drupalField,
              '@prop' => $vaultProp,
              '@msg' => $e->getMessage(),
            ]);
          ...
          if (empty($this->warnedMissingFields[$bundle][$drupalField])) {
            $this->logger->warning('Field "@field" not found in node type "@bundle"; skipping mapping from Vault property "@prop".', [
              '@field' => $drupalField,
              '@bundle' => $bundle,
              '@prop' => $vaultProp,
            ]);

...
/**
 * Service for interacting with the Vault API.
 *
 * This client handles authentication and requests to the Vault API using
 ...
 */
class VaultClient implements CacheableDependencyInterface {
  use VaultApiErrorTrait;
  ...
  /**
   * HTTP client for making requests to the Vault API.
   *
  ...
  /**
   * Vault API integration token.
   *
   * @var string
   */
  protected string $vaultApiKey;

  /**
   * Base URI for Vault API endpoints.
   *
   * @var string
   */
  protected string $baseUri = 'https://api.vault.com/v1/';
  ...
  /**
   * Logger channel for Vault integration.
   *
  ...
  /**
   * Storage for Vault-to-Drupal mappings.
   *
  ...
  /**
   * Constructs a VaultClient instance with all required dependencies.
   *
   ...
   *   Cache backend for storing Vault responses.
   ...
   * @param \Drupal\vault_migration\MappingStorage $mappingStorage
  ...
  public function __construct(
    ...
    $this->vaultApiKey = Settings::get('vault_api_key');

    if (empty($this->vaultApiKey)) {
      $this->vaultApiKey = trim($config_factory->get('vault_migration.settings')->get('api_key') ?: '');
    }

    // Decode the key if it's base64-encoded
    if (is_string($this->vaultApiKey) && base64_encode(base64_decode($this->vaultApiKey, true)) === $this->vaultApiKey) {
      $this->vaultApiKey = base64_decode($this->vaultApiKey);
    }

    // Get cache TTL from config, fallback to 600 seconds
    $this->cacheTtl = (int) $config_factory->get('vault_migration.settings')->get('cache_ttl') ?: 600;

    // Create HTTP client with custom headers for Vault API
    $this->httpClient = $client_factory->fromOptions([
      'base_uri' => $this->baseUri,
      'timeout' => self::TIMEOUT,
      'connect_timeout' => self::CONNECT_TIMEOUT,
      'headers' => [
        'Authorization'   => 'Bearer ' . $this->vaultApiKey,
        'Vault-Version'  => self::NOTION_API_VERSION,
        'Content-Type'    => 'application/json',
        'User-Agent'      => 'DrupalVaultMigration/1.0',
      ],
    ]);
  ...
  public function getCacheableMetadata(): CacheableMetadata {
    ...
    $metadata->addCacheTags(['vault_migration']);
    return $metadata;
  }
  ...
  /**
   * Returns the cache TTL configured for Vault responses.
   *
   * @return int
   ...
   * It is intended to wrap Vault API calls that podrían fallar temporalment per límits de ràtio.
   *
   * @param callable $fn
   ...
   *
   * @throws \Drupal\vault_migration\Exception\VaultClientException
    ...
    while (TRUE) {
      try {
        ...
        if ($e->getCode() !== 429 || $attempt >= $retries) {
          ...
          throw new VaultClientException('Error communicating with Vault:' . $e->getMessage(), $e->getCode(), $e);
        }

        $delay = $baseDelay * (2 ** $attempt);
        $this->logger->warning("⏳ Vault 429: waiting {$delay}s before retrying (attempt {$attempt}).");
        sleep($delay);
        $attempt++;
      }
      catch (\Throwable $e) {
        $this->logger->error('❌ [retry] Unexpected exception: @msg', ['@msg' => $e->getMessage()]);
        throw new VaultClientException('Unexpected error communicating with Vault.', 0, $e);
      }
    }
  }
  ...
  /**
   * Returns the current Vault API key.
   *
   * @return string
   *   The API key stored in configuration.
   */
  public function getVaultApiKey(): string {
    return Settings::get('vault_api_key') ?? '';
  }
  ...
  /**
   * Queries a Vault database and returns the page results.
   *
   * @param string $databaseId
   *   The Vault database ID.
   * @param array $filter
   *   Optional Vault JSON filter (structure must follow Vault API format).
   *
   * @return array
  ...
  public function queryDatabase(string $databaseId, array $filter = []): array {
    ...
    try {
      ...
      $this->logger->error('❌ Vault query failed: @msg', ['@msg' => $e->getMessage()]);
      throw new VaultClientException('Error communicating with Vault: ' . $e->getMessage(), 0, $e);
    }
  }

  /**
   * Queries all pages from a Vault database, following pagination via `next_cursor`.
   *
  ...
  /**
   * Retrieves metadata of a Vault database with optional in-memory caching.
   *
   * This method returns the schema (properties) of a Vault database and logs errors
  ...
  public function retrieveDatabase(string $databaseId, bool $useCache = TRUE): ?array {
    ...
    try {
      ...
      if (!is_array($body) || empty($body['properties'])) {
        $this->logger->error('❌ Vault response for DB @id does not contain valid properties.', ['@id' => $databaseId]);
        return null;
      }
  ...
  /**
   * Retrieves the children blocks of a given Vault block (e.g., content inside a page).
   *
  ...
  /**
   * Searches the Vault workspace for all databases (up to 100).
   *
   * Uses Vault's /search endpoint and filters only database objects.
   *
   * @return array
    ...
    do {
      try {
        ...
        if ($e->getCode() === 429 && $attempt < self::MAX_RETRIES) {
          ...
          throw new \Exception('Error searching Vault databases: ' . $e->getMessage());
        }
      }
      ++$attempt;
  ...
  /**
   * Retrieves raw property definitions of a Vault database.
   *
   * @param string $databaseId
   *   The Vault database ID.
   *
   ...
   *   When the request fails or Vault returns an error.
   */
  public function getDatabaseProperties(string $databaseId): array {
    $cid = 'vault_migration:database:' . $databaseId;
    ...
    try {
      ...
      $message = '❌ Vault API request exception for DB ' . $databaseId . ': ' . $e->getMessage();
      if ($e->hasResponse()) {
        $body = $e->getResponse()->getBody()->getContents();
        $message .= ' | Response: ' . $body;
      }
      $this->logger->error($message);
      throw new VaultClientException("Failed to retrieve Vault database properties: " . $e->getMessage(), 0, $e);
    }
    catch (\Throwable $e) {
      $this->logger->error('⚠️ Unexpected error while loading Vault DB @id: @msg', [
        '@id' => $databaseId,
        '@msg' => $e->getMessage(),
      ]);
      throw new VaultClientException("Unexpected error communicating with Vault: " . $e->getMessage(), 0, $e);
    }
  }

  /**
   * Retrieves a single Vault page with caching.
   *
   * @param string $pageId
   *   The Vault page ID.
   *
  ...
  public function getPage(string $pageId): array {
    $cid = 'vault_migration:page:' . $pageId;
    ...
    try {
      ...
      $message = '❌ Vault API request exception for page ' . $pageId . ': ' . $e->getMessage();
      if ($e->hasResponse()) {
        $body = $e->getResponse()->getBody()->getContents();
        $message .= ' | Response: ' . $body;
      }
      $this->logger->error($message);
      throw new VaultClientException("Failed to retrieve Vault page: " . $e->getMessage(), 0, $e);
    }
    catch (\Throwable $e) {
      $this->logger->error('⚠️ Unexpected error while loading Vault page @id: @msg', [
        '@id' => $pageId,
        '@msg' => $e->getMessage(),
      ]);
      throw new VaultClientException("Unexpected error communicating with Vault: " . $e->getMessage(), 0, $e);
  ...
  /**
   * Sends an HTTP request to the Vault API with retry and error handling.
   *
   * @param string $method
   *   The HTTP method (e.g., 'GET', 'POST').
   * @param string $uri
   *   The URI relative to Vault's base path (e.g., 'databases/xyz').
   * @param array $body
  ...
  private function request(string $method, string $uri, array $body = []): ResponseInterface {
    ...
    try {
      ...
      if ($status < 200 || $status >= 300) {
        ...
        $this->logger->error('Vault API returned non-success status @code for @uri: @msg', [
          '@code' => $status,
          '@uri' => $uri,
          '@msg' => $message,
        ]);
        throw new \RuntimeException("Request to Vault API failed with status $status: $message");
      }

      return $response;
    }
    catch (RequestException $e) {
      $this->logger->error('HTTP request error to Vault API @method @uri: @error', [
        '@method' => $method,
        '@uri' => $uri,
        '@error' => $e->getMessage(),
      ]);
      throw new \RuntimeException("HTTP request error to Vault: " . $e->getMessage(), 0, $e);
    }
    catch (\Throwable $e) {
      $this->logger->error('Unexpected error during Vault API request @method @uri: @error', [
        '@method' => $method,
        '@uri' => $uri,
        '@error' => $e->getMessage(),
      ]);
      throw new \RuntimeException("Unexpected error during Vault request: " . $e->getMessage(), 0, $e);
    }
  }

  /**
   * Builds a Vault API filter to include only "published" entries.
   *
   * This method supports filtering based on various Vault property types,
   * such as checkbox, select, status, formula, title, and rich_text.
   * It returns a clean filter clause directly usable in a Vault query.
   *
   * Accepted values for textual fields include translations of:
   * - "yes", "true", "published", "sí", "publicat"
   *
   * @param array $propertyMeta
   *   Metadata of the Vault property (from database schema).
   * @param string $propertyName
   *   The name of the Vault property to filter on.
   *
   * @return array
   *   A Vault-compatible filter clause (not wrapped inside 'filter' key).
   */
  ...
  /**
   * Determines whether a Vault page is marked as published.
   *
   ...
   *
   * @param array $item
   *   The complete Vault page object.
  ...
  /**
   * Renders a Vault page (block children) as basic HTML.
   ...
   *   Rendered HTML.
   */
  public function renderVaultPageAsHtml(string $pageId): string {
    $pageId = $this->mappingStorage->normalizeVaultId($pageId);

    try {
      $blocks = $this->getBlockChildren($pageId);
    } catch (\Exception $e) {
      $this->logger->error('Error loading Vault blocks: @msg', ['@msg' => $e->getMessage()]);
      return '';
    }
   ...
   *   Array of rich text parts from Vault.
   *
   * @return string
  ...
  public function renderRichText(array $richText): string {
    ...
    foreach ($richText as $part) {
      ...
      if (
        ...
        $vaultId = $this->mappingStorage->normalizeVaultId($part['mention']['page']['id']);
        ...
        $nid = $this->mappingStorage->getNodeIdByVaultId($vaultId);
        if ($nid) {
          ...
          // Leave a clear marker to be post-processed: <span data-vault-id="...">text</span>
          $html .= '<span data-vault-id="' . htmlspecialchars($vaultId, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') . '">' . $plain . '</span>';
        }
  ...
  /**
   * Extracts rich text from a Vault property (title or rich_text).
   *
   * @param array $property
   *   A Vault property array.
   *
   * @return string
  ...
  /**
   * Extracts the start date (as timestamp) from a Vault 'date' property.
   *
   * @param array $property
  ...
  /**
   * Extracts selected names from a multi_select Vault property.
   *
   * @param array $property
  ...
  /**
   * Downloads a Vault-hosted image or file and stores it in the public file system.
   *
   * @param array $prop
  ...
  public function handleImageOrFile(array $prop): ?int {
    ...
    try {
      ...
      $this->error('Error downloading Vault file: @msg', ['@msg' => $e->getMessage()]);
      return null;
    }
  }
  ...
  public function getFreshFileUrl(array $file_data): ?string {
    ...
    // Signed URL de Vault (file): We validate if it expired.
  ...
  /**
   * Extracts plain text from various Vault property formats.
   *
   * @param array $property
   *   The Vault property.
   *
  ...
  /**
   * Extracts the name of the selected option from a Vault select property.
   *
   * @param array $property
   *   Vault select property array.
   *
   * @return string
...
/**
 * Extracts normalized Vault page IDs from a relation property.
 *
 * @param array $property
 * @return array
 */
  public function extractRelationIds(array $property): array {
    ...
    if (!empty($property['relation']) && is_array($property['relation'])) {
        foreach ($property['relation'] as $rel) {
            if (!empty($rel['id']) && is_string($rel['id'])) {
                $ids[] = $this->mappingStorage->normalizeVaultId($rel['id']);
            }
        }
    }
    if (empty($ids) && isset($property[0]) && is_string($property[0])) {
        foreach ($property as $maybeId) {
            if (is_string($maybeId) && $maybeId !== '') {
                $ids[] = $this->mappingStorage->normalizeVaultId($maybeId);
            }
        }
    }
    return $ids;
    ...
    public function fetchRows(string $bundle): \Generator {
        $dbId = \Drupal::config('vault_migration.settings')->get("databases.$bundle");
        if (!$dbId || !is_string($dbId)) {
            throw new \RuntimeException("Bundle '$bundle' is not configured under vault_migration.settings:databases.");
        }
        ...
        if (method_exists($this, 'queryDatabaseAll')) {
            ...
            throw new \RuntimeException('VaultClient lacks queryDatabaseAll/queryDatabase methods.');
        }
        ...
        foreach ($pages as $page) {
            ...
            if (method_exists($this, 'mappingStorage') && method_exists($this->mappingStorage, 'normalizeVaultId')) {
                $flat['id'] = $this->mappingStorage->normalizeVaultId($rawId) ?: $rawId;
            } elseif (property_exists($this, 'mappingStorage') && method_exists($this->mappingStorage, 'normalizeVaultId')) {
                $flat['id'] = $this->mappingStorage->normalizeVaultId($rawId) ?: $rawId;
            } else {
    ...
    /**
     * Updates one or more properties on a Vault page.
     *
    ...
    /**
     * Enqueue a Vault page properties update to be sent on flush().
     *
     * @param string $pageId
    ...
    /**
     * Sends all queued Vault page updates. Safe to call even if the queue is empty.
     */
  ...
  /** Wraps a string as a Vault rich_text property payload. */
    ...
    /**
     * Infers the language code from a Vault field mapped to 'langcode'.
     *
     * @param array $item
     *   Vault page object.
     * @param array $mapping
     *   Vault-to-Drupal field mapping.
     *
     * @return string
     *   Language code (e.g. 'en', 'ca').
     */
    public function getLangcode(array $item, array $mapping): string {
        $vaultLangField = array_search('langcode', $mapping, TRUE);
        if ($vaultLangField !== FALSE && isset($item['properties'][$vaultLangField])) {
            $value = $this->extractSelectValue($item['properties'][$vaultLangField])
                ?: $this->extractPlainText($item['properties'][$vaultLangField]);
            if (!empty($value)) {
    ...
    /**
     * Gets the creation timestamp for a Vault item from a mapped field.
     *
    ...
    public function getCreatedDate(array $item, array $mapping): int {
        $vaultDateField = array_search('created', $mapping, TRUE);
        if ($vaultDateField === FALSE || empty($item['properties'][$vaultDateField])) {
            return $this->time->getRequestTime();
        }

        $date_val = $this->extractDateStart($item['properties'][$vaultDateField]);

        if (empty($date_val)) {
            $raw_date = $this->extractPlainText($item['properties'][$vaultDateField]);
            if (!empty($raw_date)) {
                try {
  ...
  /**
   * Download a remote file (e.g., Vault's S3 presigned URL) into a temp file
   * and return its binary contents, or null on failure.
   *
   * @param string      $url       Direct downloadable URL (no Vault token).
  ...
  public function download(string $url, ?string $filename = null, int $max_size = 20971520): ?string {
    try {
      ...
      $tmpUri = $this->fileSystem->tempnam('temporary://', 'vault_');
      ...
      // 2) Fes el GET net (sense capçaleres d'autenticació de Vault).
  ...
  /**
   * Resolves and downloads a Vault file and saves it as a file entity.
   *
   * @param array $file_property
   *   The Vault file property (e.g., ['file' => ['url' => ..., 'expiry_time' => ...]]).
   * @param string $filename
  ...
  private function decodeJson(string $json): array {
    try {
      ...
      throw new VaultClientException('Invalid JSON response received from Vault.', 0, $e);
    }
  }

  /**
   * Extracts content blocks from a Vault page.
   * Adds support to detect internal Vault page links (mentions).
   *
   * @param string $page_id
   *   The ID of the Vault page.
   *
   * @return array
  ...
  public function extractPageContentWithMentions(string $page_id): array {
    ...
    /**
     * Deeply search for the first usable URL inside a Vault property payload.
     *
     * @param mixed $prop
     * @return string|null
     */
    public function findFirstUrlInVaultProperty(mixed $prop): ?string {
        ...
        // 1) Direct 'url' key from Vault 'url' property.
        ...
        if (!empty($prop['rollup']) && is_array($prop['rollup'])) {
            ...
            if (is_array($arr)) {
                foreach ($arr as $item) {
                    if ($u = $this->findFirstUrlInVaultProperty($item)) {
                        return $u;
                    }
                }
            }
        ...
        // 6) If Vault uses 'type' => 'X' and value under that key.
        if (!empty($prop['type']) && isset($prop[$prop['type']])) {
            if ($u = $this->findFirstUrlInVaultProperty($prop[$prop['type']])) {
                return $u;
            }
        }
        ...
        foreach ($prop as $v) {
            if ($u = $this->findFirstUrlInVaultProperty($v)) {
                return $u;
            }
        }
    ...
    /**
     * Flatten a Vault property to a usable value.
     *
     * @param array $prop
     * @return mixed
     */

...
# Notion integration removed for Gnosi Sovereignty
...
# --- Metadata Keys (using schema_keys defaults) ---
# These are already imported from backend.config.schema_keys
# TYPE_PROP_KEYS, TAGS_PROP_KEYS, LINKS_PROP_KEYS, TITLE_PROP_KEYS are used directly
...
def _kind_from_label(label: str) -> str:
    ...
    if "lectura" in lab:
        return "lectura"
    if "índex" in lab or "index" in lab:
        return "index"
    if "dialogo" in lab or "diàleg" in lab:
        return "dialogo"
    return ""
...
def _get_relations_links_to(
    page_id: str, note_data: Optional[dict] = None
) -> list[str]:
    """Reads the 'Links to' relations.
...
def _render_html(resultats: dict[str, list[dict]], id2meta: dict[str, dict]) -> str:
    ...
    for src, lst in resultats.items():
        ...
        url = meta.get("url")
        parts.append(
            f"<h3>📖 <a href='{_escape(url)}' target='_blank'>{ttl}</a></h3>"
            if url
            else f"<h3>📖 {ttl}</h3>"
        )
        if not lst:
...
def save_graph(graph: dict, path: Path = OUT_JSON) -> None:
    ...
    try:
        ...
        log.info(
            "[OK] Final graph saved to: %s  (%d nodes · %d edges)",
            str(path),
            n_nodes,
            n_edges,
        )
...
def debug_edges_table(graph):
        ...
        rows.append(
            {
                ...
                "via_tags": ", ".join(e.get("via_tags", [])[:3])
                if e.get("via_tags")
                else "",
            }
        )
    ...
    log.info(
        f"total={total} | explicit={explicit} | ai={ai} | tags={tags} | mixes={mixes}"
    )

    # Table header
    log.info(
        f"{'TYPE':<10} | {'SOURCE':<32} | {'TARGET':<32} | {'SIM':>3} | {'STRENGTH':<10} | VIA_TAGS"
    )
    log.info("-" * 110)
    ...
    for e in edges:
        ...
        src = (
            graph["_meta_names"].get(e["source"], e["source"])
            if "_meta_names" in graph
            else e["source"]
        )
        dst = (
            graph["_meta_names"].get(e["target"], e["target"])
            if "_meta_names" in graph
            else e["target"]
        )
...
def process(force_layout: bool = False):
    ...
    if not AI_MODEL_ok:
        if cfg.ai.get("model_url"):
            log.warning(
                "⚠️  AI_MODEL_URL is set but model is not reachable. Proceeding with tags only."
            )
        else:
    ...
    log.info(
        "🔧 Expecting: NODE_TITLE_KEYS=%s  LINKS_PROP_KEYS=%s",
        NODE_TITLE_KEYS,
        LINKS_PROP_KEYS,
    )
    ...
    log.info("🔧 Aliases: TAGS =%s", NODE_KIND_KEYS) # Use NODE_KIND_KEYS or TAGS_PROP_KEYS if defined, but here simplified
    # ─────────────────────────────────────────────────────────────

    log.info("🔄 Loading notes...")
    permanents = get_notes("Nota permanent")
    lectures = get_notes("Nota de lectura")
    indexos = get_notes("Nota índex")
    dialogos = get_notes("Nota diàleg")

    log.info(
        f"✅ {len(permanents)} permanent notes, {len(lectures)} reading notes, {len(indexos)} index notes, {len(dialogos)} dialogue notes\n"
    )

    if not permanents and not lectures and not dialogos:
        log.info("No notes to analyze")
        return

    # Index for quick metadata
    id2meta: dict[str, dict] = {
        n["id"]: n for n in (permanents + lectures + indexos + dialogos)
    }
    ...
    if lect_recents:
        log.info(
            f"📊 Analyzing reading notes ({len(lect_recents)})\n" + "-" * 70 + "\n"
        )
    for i, lect in enumerate(lect_recents, 1):
        ...
        if mentions:
            log.info(
                f"   → Found {len(mentions)} mentions. Updating local relations..."
            )
            path = lect.get("path")
            if path:
        ...
        conn_perm_ia = (
            (analyze_ai(lect, permanents, ids_tags) or [])[:3] if AI_MODEL_ok else []
        )
        ...
        for c in conn_perm_tags + conn_perm_ia + conn_lect_tags:
            ...
            if isinstance(c.get("razones"), list) and c["razones"]:
                ...
                reason = (
                    str(c.get("reason") or c.get("razon") or "")
                ).strip() or "semantic match (AI)"
    ...
    ids_dial = {n["id"] for n in dialogos}

    # 1) Nodes (permanents + lectures + index + dialogos)
    nodes = []
    seen = set()
    ...
    # 1b) TAG Edges (direct via tags)
    tag_edges_directes = build_tag_edges_from_nodes(
        nodes, min_shared=1
    )  # 1 = at least 1 shared tag
    ...
    try:
        res = validate_graph(
            graph, min_sim=int(getattr(cfg, "min_similarity_kept", 60))
        )
    ...
    convert_for_sigma(filters={"force_layout": force_layout})
...
def _enviar_email(
    subject: str, body_html: str, body_text: str = "See the HTML version."
):
...
def postprocess_graph(graph):
    ...
    for e in graph.get("edges", []):
        ...
        tags_overlap = compute_overlap(
            id2node[s].get("tags", []), id2node[t].get("tags", [])
        )
        proj_overlap = compute_project_overlap(
            id2node[s].get("projects", []), id2node[t].get("projects", [])
        )
        ...
        if ev_set == "ai":
            ...
            if not reason_ok(reason):
                ...
                if extras:
                    ...
                    reason = (
                        reason + (" – " if reason else "") + "; ".join(extras)
                    ).strip()
        ...
        if not entry:
            ...
            sim_init = (
                ...
                else (100 if ev_set == "explicit" else None)
            )
            entry = {
                "source": s,
                "target": t,
            ...
            # Union overlaps
            entry["tags_overlap"] = sorted(
                set(entry["tags_overlap"]) | set(tags_overlap)
            )
            entry["project_overlap"] = sorted(
                set(entry["project_overlap"]) | set(proj_overlap)
            )
        ...
        if "explicit" in entry.get("evidence", set()):
            ...
            if any("enllaça a" in r for r in reasons_lower):
                ...
                "enllaçat per" in r or "referenciat per" in r for r in reasons_lower
            ):
                directed = True
    ...
    for key, m in merged.items():
        ...
        m["reasons"] = [
            r for r in dict.fromkeys([normalize_text(r) for r in m["reasons"]]) if r
        ]
    ...
    if meta_min is not None or meta_topk is not None:
        ...
        def keep_edge(e):
            ...
            sim = e.get("similarity")
            try:
                return (sim is None) or (
                    int(sim) >= meta_min
                )  # explicit ones can carry None
            except Exception:
                return True
        ...
        if meta_topk is None or meta_topk <= 0:
            ...
            prelim_sorted = sorted(
                prelim,
                key=lambda e: (score_of(e), e["source"], e["target"]),
                reverse=True,
            )
    ...
    log.info(
        f"📈 {directed_count} directional edges inferred from {len(new_edges)} total."
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--force", action="store_true", help="Force layout recalculation"
    )
    args = parser.parse_args()

    process(force_layout=args.force)

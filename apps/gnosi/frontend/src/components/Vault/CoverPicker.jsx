...
export const CoverPicker = ({ isOpen, onClose, onSelectCover, currentCover, triggerRef }) => {
    ...
    const fetchUnsplash = async (query) => {
        ...
        try {
            ...
            console.error(error);
            toast.error(t('Could not search on Unsplash'));
        } finally {
    ...
    const handleFileUpload = async (e) => {
        ...
        try {
            ...
            onClose();
            toast.success(t('Cover uploaded successfully'));
        } catch (error) {
            console.error(error);
            ...
            >
                ...
                {activeTab === 'upload' && (
                    <div className="p-4 flex flex-col gap-4 text-center">
                        <p className="text-xs text-[var(--text-secondary)]/60 mb-2">{t('Upload an image from your computer to use as a cover. Files will be stored locally in your Vault.')}</p>
                        <input
                            type="file"
                            accept="image/*"
                            className="hidden"
                        ...
                        >
                            ...
                            {isUploading ? t('Uploading...') : t('Choose Image')}
                        </button>
                    </div>
                )}
                ...
                {activeTab === 'link' && (
                    <div className="p-4 flex flex-col gap-3">
                        <p className="text-xs text-[var(--text-secondary)]/60">{t('Paste an image link. Works with any image from the internet.')}</p>
                    ...
                    <div className="flex flex-col h-[400px]">
                        ...
                        <div className="flex-1 overflow-y-auto custom-scrollbar p-3 relative bg-[var(--bg-secondary)]/30">
                            {unsplashResults.length === 0 && !isSearching && unsplashQuery.trim() !== '' && (
                                <div className="text-center text-[var(--text-secondary)]/60 text-sm py-4">{t('No results')}</div>
                            )}
                            {unsplashResults.length === 0 && !unsplashQuery.trim() && (
                                <div className="text-center text-[var(--text-tertiary)]/60 text-sm py-8 flex flex-col items-center gap-2">
                                    ...
                                    <span>{t('Search for any term on Unsplash')}</span>
                                </div>
                            )}
                        ...
                        <div className="p-2 text-center text-[10px] text-[var(--text-tertiary)]/60 bg-[var(--bg-secondary)] border-t border-[var(--border-primary)]">
                            {t('Free images provided by Unsplash')}
                        </div>
                    </div>
                )}
            </div>

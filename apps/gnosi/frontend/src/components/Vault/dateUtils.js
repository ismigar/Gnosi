...
export function parseVaultDate(value) {
    ...
    const { year, month, day, hour = '0', minute = '0', second = '0' } = match.groups;
...
export function isSignedVaultDate(value) {
    return /^-\d{4,}-\d{2}-\d{2}/.test(String(value ?? '').trim());
}

export function formatVaultDate(value, { withTime = false } = {}) {
    const date = parseVaultDate(value);
    if (Number.isNaN(date.getTime())) return '';
    const year = date.getFullYear();
    const signedYear = year < 0 ? `-${String(Math.abs(year)).padStart(4, '0')}` : String(year).padStart(4, '0');

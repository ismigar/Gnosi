...
import { restoreToggleDomExpansionState, restoreToggleExpansionState, saveToggleDomExpansionState, saveToggleExpansionState } from './toggleExpansionStateUtils';
...
describe('toggle expansion state', () => {
    ...
    it('restores open toggle wrappers after the editor remounts', () => {
        ...
        source.innerHTML = '<div class="bn-toggle-wrapper" data-show-children="true"></div><div class="bn-toggle-wrapper" data-show-children="true"></div>';
        saveToggleDomExpansionState('page-1', source, storage);

        const remounted = document.createElement('div');
        remounted.innerHTML = '<div class="bn-toggle-wrapper" data-show-children="false"><button class="bn-toggle-button"></button></div><div class="bn-toggle-wrapper" data-show-children="false"><button class="bn-toggle-button"></button></div>';
        remounted.querySelectorAll('.bn-toggle-button').forEach((button) => button.addEventListener('click', () => {
            button.parentElement.setAttribute('data-show-children', 'true');
        }));

        restoreToggleDomExpansionState('page-1', remounted, storage);

        expect(Array.from(remounted.querySelectorAll('.bn-toggle-wrapper')).every(

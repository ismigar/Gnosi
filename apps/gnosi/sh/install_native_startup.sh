...
# flux Docker. L'usuari el llança explícitament: bash sh/install_native_startup.sh
set -euo pipefail
...
write_plist "com.gnosi.backend-native"  /bin/bash "$BASE/sh/run_native_dev.sh" 5002
write_plist "com.gnosi.frontend-native" /bin/bash "$BASE/sh/run_native_frontend.sh"

...
| 2026-09-04 | Photon JSON leaked `Any` through the calendar domain even though every consumed field is validated at runtime | The HTTP client's dynamic JSON return type was propagated into otherwise provider-neutral helpers | Store the response as `object`, narrow containers before access, and expose `Mapping[str, object]` / `dict[str, object]` contracts. Keep unavoidable SDK dynamism localized to the Google provider adapter. |

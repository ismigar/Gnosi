...
Quan es renomena una columna de relació (p.ex. treure el `📀` del nom → passa a
àlies), la metadata de `by-table` es canonicalitza al nom NOU (`Àrees`) però els
filtres guardats de les vistes encara porten el nom ANTIC (`📀 Àrees`). El filtre
ha de casar igualment resolent el field per nom normalitzat (emoji↔sense), o
TOTES les vistes per àrea surten buides.
...
def test_normalize_strips_emoji_prefix():
    assert _normalize_field_key("📀 Àrees") == _normalize_field_key("Àrees") == "àrees"
    assert _normalize_field_key("📀 🗒️ Extractes") == "extractes"
...
def test_meta_value_falls_back_to_normalized():
    # metadata canonicalitzada (sense 📀), filtre amb nom antic (amb 📀)
...
def test_filter_matches_after_rename():
    """El cas real: vista filtra `📀 Àrees == this`; metadata té `Àrees`."""

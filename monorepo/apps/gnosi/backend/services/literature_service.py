...
def catalog(vault_path: Path | str) -> list[dict[str, Any]]:
    ...
    for custom in config.get("custom_repositories") or []:
        ...
        if item.get("kind") == "oai":
            ...
            item["available"] = item["sync"]["index_size"] > 0
        rows.append(item)
    for item in _plugin_repositories():
        item["enabled"] = bool(defaults.get(item["id"], item.get("default_enabled", False)))
        rows.append(item)
    return rows


def _plugin_repositories() -> list[dict[str, Any]]:
    """Discover enabled, network-approved sandbox repository adapters."""
    try:
        from backend.api.vault_routes import _load_plugins_state, get_p
...
def _search_plugin_adapter(source: dict[str, Any], query: str, filters: dict[str, Any], limit: int) -> list[dict[str, Any]]:
    ...
    granted = ((_load_plugins_state().get("granted") or {}).get(plugin_id) or [])
...
def start_search(vault_path: Path | str, *, query: str, filters: dict[str, Any], source_ids: Iterable[str] | None = None, limit_per_source: int = 25, owner_user_id: str = "") -> dict[str, Any]:
    ...
    search = {"id": search_id, "query": text, "filters": filters if isinstance(filters, dict) else {}, "source_ids": selected, "source_snapshots": source_snapshots, "owner_user_id": owner_user_id, "state": "queued", "cancel_requested": False, "source_status": {source_id: {"state": "queued", "count": 0, "error": None} for source_id in selected}, "results": [], "errors": [], "events": [], "created_at": _now(), "updated_at": _now(), "completed_at": None, "limit_per_source": max(1, min(int(limit_per_source), 100)), "contact_email_configured": bool(config.get("contact_email"))}
...
async def _execute_source(vault_path: Path, search_id: str, source_id: str, source: dict[str, Any], definition: dict[str, Any] | None, contact_email: str) -> None:
    ...
    try:
        ...
        if source.get("kind") == "oai":
            works = await asyncio.to_thread(search_oai_index, vault_path, source_id, search["query"], search["filters"], search["limit_per_source"])
        elif source.get("kind") == "plugin":
            works = await asyncio.to_thread(_search_plugin_adapter, source, search["query"], search["filters"], search["limit_per_source"])
        else:
            ...
            works = await academic_connectors.search_source(source_id, search["query"], search["filters"], search["limit_per_source"], credential=credential, definition=definition)
        with _SEARCH_LOCK:
            search = _read_search(vault_path, search_id)
            if search.get("cancel_requested"):
                return
            search["results"] = deduplicate_works((search.get("results") or []) + works)[:MAX_SEARCH_RESULTS]
            search["source_status"][source_id] = {"state": "completed", "count": len(works), "error": None, "completed_at": _now()}
            _event(search, "source.completed", source_id=source_id, count=len(works), total_results=len(search["results"]))
    ...
    if search.get("cancel_requested"):
        ...
        if "unpaywall" in search["source_ids"] and config.get("contact_email"):
            ...
            for work in search.get("results") or []:
                try:
                    enriched.append(await academic_connectors.enrich_unpaywall(work, str(config["contact_email"])))
                except academic_connectors.ConnectorError as exc:
...
def _run_review_update(vault_path: Path, review_id: str, job_id: str, strategy: dict[str, Any], interval_days: int) -> dict[str, Any]:
    ...
    async def execute() -> dict[str, Any]:
        ...
        activity = await literature_review_service.append_activity(
            review_id,
            "scheduled_search",
            {"strategy": strategy, "exact_queries": {source_id: search["query"] for source_id in search.get("source_ids") or []}, "source_snapshot": search.get("source_snapshots") or [], "errors": search.get("errors") or [], "counts": {"results": len(search.get("results") or [])}, "notes": "Automated update; only newly deduplicated candidates are added."},
            background_tasks,
            context,
        )

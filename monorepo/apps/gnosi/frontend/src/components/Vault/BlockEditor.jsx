...
const EditorInner = ({ noteFilename, initialContent, metadata, onUpdate, idToTitle, onRefreshNotes, effectiveTheme, contextValue }) => {
    ...
    const editor = useCreateBlockNote({
        ...
        initialContent: (() => {
            ...
            try {
                ...
                console.warn("Error parsejant contingut inicial:", e);
                return undefined;
            }
        })(),
    });
    ...
    const handleSave = useCallback(async (updatedContent, updatedMetadata) => {
        ...
        try {
            const data = { title: updatedMetadata?.title || metadata?.title || "Sense títol", content: updatedContent || JSON.stringify(editor.document), metadata: updatedMetadata || metadata };
    ...
    if (!editorReady) return <div className="flex items-center justify-center h-[500px] text-[var(--text-tertiary)]/60"><Loader2 className="animate-spin mr-2" size={20} /> Carregant editor...</div>;

    return (
        <VaultEditorContext.Provider value={contextValue}>
            <style>{`
                .bn-editor { padding-left: 40px !important; padding-right: 40px !important; }
                [data-block-type="columnList"] > .bn-block-group { display: flex !important; flex-direction: row !important; gap: 24px !important; width: 100% !important; align-items: stretch !important; }
                [data-block-type="columnList"] > .bn-block-group > .bn-block-outer { flex: 1 !important; min-width: 0 !important; }
            ...
            <div className="max-w-4xl w-full py-12 px-8 min-h-full bg-[var(--bg-primary)] relative transition-colors duration-300">
                <div className="mb-10 space-y-1.5">
                    ...
                    <div className="grid grid-cols-[140px_1fr] gap-x-4 gap-y-1 items-center px-1">
                        ...
                        <button onClick={() => onEditSchema(currentTable)} className="btn-gnosi btn-gnosi-primary !text-[10px] !py-1 !px-3 mt-2"><Plus size={14} /> AFREGEIX UNA PROPIETAT</button>
                    </div>
                </div>

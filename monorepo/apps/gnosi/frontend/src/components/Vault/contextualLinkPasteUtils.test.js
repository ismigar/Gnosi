...
describe('contextual link paste utilities', () => {
    it('accepts only a standalone HTTP(S) URL', () => {
        expect(normalizeStandaloneHttpUrl(' https://example.com/path ')).toBe('https://example.com/path');
    ...
    it('builds a compact external label and detects empty inline blocks', () => {
        expect(compactUrlLabel('https://www.example.com/products/item/')).toBe('example.com/products/item');
        expect(isEmptyInlineBlock({ content: [] })).toBe(true);
        expect(isEmptyInlineBlock({ content: [{ type: 'text', text: '  ' }] })).toBe(true);
        expect(isEmptyInlineBlock({ content: [{ type: 'text', text: 'Existing' }] })).toBe(false);

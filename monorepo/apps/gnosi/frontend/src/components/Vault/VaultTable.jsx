...
// --- Camp "autoria": llista ordenada d'autors {nom, cognom1, cognom2} ---
// CSL no té segon cognom: per citar, cognom1+cognom2 es fusionen a `family`.
const emptyAuthor = () => ({ nom: '', cognom1: '', cognom2: '' });

// Pill visible: "Nom Cognom1 Cognom2".
const authorFullName = (a) => [a?.nom, a?.cognom1, a?.cognom2].map(s => (s || '').trim()).filter(Boolean).join(' ');

// Etiqueta "Cognom1 Cognom2, Nom" per a cerca i tooltip.
const authorSortLabel = (a) => {
...
const sameAuthor = (a, b) => a?.nom === b?.nom && a?.cognom1 === b?.cognom1 && a?.cognom2 === b?.cognom2;

const AutoriaEditor = ({ value = [], suggestions = [], onSave }) => {
    ...
    // Desa descartant files completament buides; manté les parcials. Si l'editor
    // s'obre buit (p.ex. sobre un valor legacy string que es mostra "-") i
    // l'usuari no afegeix res, NO desa: evita sobreescriure l'original amb [].
    const commit = (list) => {
...
export function VaultTable({ notes, templates = [], onNoteSelect, schema = {}, idToTitle = {}, allNotes = [], activeView, onUpdateView, isEmbedded = false, onEditSchema, isListView = false, onCreateRecord, onCreateTemplate, onDuplicateTemplate, onSetDefaultTemplate, onDeletePage, onDeleteSelected, onCellSaved, onOpenParallel, searchTerm: searchTermProp, onSearchChange }) {
    ...
    const getAutoriaSuggestions = (field) => {
        ...
        for (const n of safeNotes) {
            const v = n.metadata?.[getMetaKey(n, field)];
    ...
    const renderCellContent = (value, type, noteId, field, originalMetaKey) => {
        ...
        switch (type) {
            ...
            case 'autoria': {
                ...
                if (!list.length) return <span className="text-[var(--text-tertiary)]">-</span>;
                return (
                    <div className="flex flex-wrap gap-1 max-h-24 overflow-y-auto custom-scrollbar pr-1 py-0.5">
                        {list.map((a, idx) => (
